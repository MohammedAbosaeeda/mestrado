...
public class EpsGraphics extends java.awt.Graphics2D {
    ...
    public EpsGraphics(String title, OutputStream outputStream, int minX, int minY, int maxX, int maxY,
            int colorMode) throws IOException {
    ...
    public EpsGraphics(EpsDocument doc, int m)throws IOException{
        new EpsGraphics(doc.getTitle(),doc.getStream(),(int)doc.getMinX(),(int)doc.getMinY(),(int)doc.getMaxX(),(int)doc.getMaxY(),colorMode);
    ...
    public boolean drawImage(Image img, int dx1, int dy1, int dx2, int dy2, int sx1, int sy1, int sx2, int sy2,
        ...
        setColor(oldColor);
        if (this.colorMode == ColorMode.BLACK_AND_WHITE || this.colorMode == ColorMode.GRAYSCALE) {
        ...
        for (int y = 0; y < height; y++) {
            for (int x = 0; x < width; x++) {
                ...
                } else if (this.colorMode == ColorMode.GRAYSCALE) {
    ...
    /**
     * Disposes of all resources used by this EpsGraphics object. If this is
     * the only remaining EpsGraphics instance pointing at a EpsDocument
    ...
    private int colorMode = ColorMode.COLOR_RGB;
