...
void Single_Core_Alarm::delay(const Microsecond & time)
{
	...
	kout << "Alarm::delay before semaphore.p()\n";
	semaphore.p();
    kout << "Alarm::delay after semaphore.p()\n";
    } else {

    kout << "Alarm::delay busy waiting..";
...
{
    CPU::int_disable();
    kout << "Single_Core_Alarm::handler()\n";
    // lock(); this handler is meant to be called only by CPU[0]
