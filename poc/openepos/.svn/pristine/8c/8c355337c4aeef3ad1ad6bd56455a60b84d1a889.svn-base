...
public:
    SMP_Alarm(const Microsecond & time, Handler * handler, int times = 1, int queue = -1);
    ~SMP_Alarm();
    ...
    static volatile Tick get_elapsed() { return _elapsed[Machine::cpu_id()]; }
    
    //static void delete_timer() { delete _timer; }
    //static void create_timer() { _timer = new (kmalloc(sizeof(Alarm_Timer))) Alarm_Timer(&handler); }

private:
    //static void insert(Queue::Element *e);
    //static Queue::Element * remove(void);
    static void handler();
public:
    //static const unsigned int MAX_ELAPSED = 100000;
    static volatile Tick _elapsed[MAX_CPUS];
    //static volatile TSC::Time_Stamp _tsc[MAX_CPUS][MAX_ELAPSED];
    //static volatile Tick _tick_couting[MAX_CPUS];
private:
    Queue::Element _link; 
