...
template<> 
void CMAC<Radio_Wrapper>::cmac_yield() {
	Thread::yield();
}
template<> 
CMAC<Radio_Wrapper>::CMAC_STATE_TRANSITION CMAC<Radio_Wrapper>::state_machine() {
    ...
    db<CMAC>(WRN) << "CMAC::state_machine - starting state machine\n";
    ...
    while (_state != OFF) {
        switch (_state) {
            ...
            case ASYNCHRONOUS_SYNC:
				if(Traits<CMAC<Radio_Wrapper> >::csma || _rx_pending)
				{
//					kout << "Okay1" << endl;
					result = Asynchronous_Sync<Radio_Wrapper>::execute(RX_PENDING);
//					kout << result << endl;
					//Channel idle
					if (result == TIMEOUT) 
					{
//						kout << "Okay2" << endl;
						...
						{
//							kout << "Okay" << endl;
							result = Asynchronous_Sync<Radio_Wrapper>::execute(TX_PENDING);
						}
					}
				}
				else if(_tx_pending)
					result = Asynchronous_Sync<Radio_Wrapper>::execute(TX_PENDING);
				else if(Traits<CMAC<Radio_Wrapper> >::auto_rx)
					result = Asynchronous_Sync<Radio_Wrapper>::execute(RX_PENDING);
            ...
            case TX_DATA:
				...
				//chrono->stop();
				//kout << "chrono: " << r << " " << chrono->read() << endl;
				//chrono->reset();
...
void CMAC<Radio_Wrapper>::state_machine_handler() {
    if (Traits<CMAC<Radio_Wrapper> >::time_triggered) {
        ...
        */
		...
		{
			//kout << "Notifying " << (int)_rx_protocol << endl;
		...
		else if (_state_machine_result == TX_OK)
		{
	        alarm_activate(&(CMAC<Radio_Wrapper>::state_machine_handler), _sleeping_period);
