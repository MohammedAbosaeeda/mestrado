/*! @file
 *  @brief EPOS MMU Mediator Common Package
 *
 *  CVS Log for this file:
 *  \verbinclude include/mmu_h.log
 */
#ifndef __mmu_h
#define __mmu_h
...
public:
    typedef unsigned char Page[PAGE_SIZE]; ///< Memory page
    typedef Page Frame;

    typedef Phy_Addr PT_Entry; ///< Page_Table entries
    typedef Phy_Addr PD_Entry; ///< Page_Directory entries
	...
	enum {
	    ...
	    SYS = (PRE | RW ), ///< System (Presence + Write)
	    APP = (PRE | RW | USR) ///< Application (Presence + Write + Access Control)
	};
    ...
    static const unsigned int PT_ENTRIES = sizeof(Page) / sizeof(PT_Entry); ///< Number of entries in Page_Table
    static const unsigned int PD_ENTRIES = PT_ENTRIES; ///< Number of entries in Page_Directory
