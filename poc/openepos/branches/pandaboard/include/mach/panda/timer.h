...
class Zynq_Timer: public PandaBoard_Timer
{
	...
	static void load(CPU::Reg32 val) {
        CPU::out32(PRIVATE_TIMER_LOAD, val);
    }

    static CPU::Reg32 load() {
        return CPU::in32(PRIVATE_TIMER_LOAD);
    }    

    static void control(CPU::Reg32 val) {
        CPU::out32(PRIVATE_TIMER_CONTROL, val);
    }

    static CPU::Reg32 control() {
        return CPU::in32(PRIVATE_TIMER_CONTROL);
    }

    static void status(CPU::Reg32 val) {
        CPU::out32(PRIVATE_TIMER_INTERRUPT_STATUS, val);
    }

    static CPU::Reg32 status() {
        return CPU::in32(PRIVATE_TIMER_INTERRUPT_STATUS);
    }
