// OpenEPOS EMote2ARM_Transceiver Mediator Implementation

#include <mach/emote2arm/transceiver.h>
...
volatile bool EMote2ARM_Transceiver::cca_result = false;

EMote2ARM_Transceiver::get_lqi_func EMote2ARM_Transceiver::get_lqi = reinterpret_cast<get_lqi_func>(reinterpret_cast<void *>(GET_LQI_ADDR));

EMote2ARM_Transceiver::event_handler * EMote2ARM_Transceiver::handler = 0;

volatile unsigned char EMote2ARM_Transceiver::fcs_mode = EMote2ARM_Transceiver::NO_FCS;

volatile EMote2ARM_Transceiver::packet_t EMote2ARM_Transceiver::packet_pool[NUM_PACKETS];
volatile EMote2ARM_Transceiver::packet_t *EMote2ARM_Transceiver::free_head, *EMote2ARM_Transceiver::rx_end, *EMote2ARM_Transceiver::tx_end, *EMote2ARM_Transceiver::dma_tx, *EMote2ARM_Transceiver::dma_rx;

volatile EMote2ARM_Transceiver::packet_t *EMote2ARM_Transceiver::rx_head;
volatile EMote2ARM_Transceiver::packet_t *EMote2ARM_Transceiver::tx_head;

// used for ack recpetion if the packet_pool goes empty
// doesn't go back into the pool when freed
volatile EMote2ARM_Transceiver::packet_t EMote2ARM_Transceiver::dummy_ack;

volatile unsigned char EMote2ARM_Transceiver::current_action = SEQ_NOP;

unsigned char EMote2ARM_Transceiver::ram_values[4];

const unsigned int EMote2ARM_Transceiver::addr_seq2[MAX_SEQ2] = {0x8000a050,0x8000a054};
const unsigned int EMote2ARM_Transceiver::data_seq2[MAX_SEQ2] = {0x0000047b,0x0000007b};

const unsigned int EMote2ARM_Transceiver::addr_cal3_seq1[MAX_CAL3_SEQ1] = {0x80009400,0x80009a04,0x80009a00};
const unsigned int EMote2ARM_Transceiver::data_cal3_seq1[MAX_CAL3_SEQ1] = {0x00020017,0x8185a0a4,0x8c900025};

const unsigned int EMote2ARM_Transceiver::addr_cal3_seq2[MAX_CAL3_SEQ2] = {0x80009a00,0x80009a00};
const unsigned int EMote2ARM_Transceiver::data_cal3_seq2[MAX_CAL3_SEQ2] = {0x8c900021,0x8c900027};

const unsigned int EMote2ARM_Transceiver::addr_cal3_seq3[MAX_CAL3_SEQ3] = {0x80009a00};
const unsigned int EMote2ARM_Transceiver::data_cal3_seq3[MAX_CAL3_SEQ3] = {0x8c900000};

const unsigned int EMote2ARM_Transceiver::addr_cal5[MAX_CAL5] = {0x80009400,0x8000a050,0x8000a054};
const unsigned int EMote2ARM_Transceiver::data_cal5[MAX_CAL5] = {0x00000017,0x00000000,0x00000000};

const unsigned int EMote2ARM_Transceiver::addr_reg_rep[MAX_DATA] = {0x80004118,0x80009204,0x80009208,0x8000920c,0x80009210,0x80009300,0x80009304,0x80009308,0x8000930c,0x80009310,0x80009314,0x80009318,0x80009380,0x80009384,0x80009388,0x8000938c,0x80009390,0x80009394,0x8000a008,0x8000a018,0x8000a01c,0x80009424,0x80009434,0x80009438,0x8000943c,0x80009440,0x80009444,0x80009448,0x8000944c,0x80009450,0x80009460,0x80009464,0x8000947c,0x800094e0,0x800094e4,0x800094e8,0x800094ec,0x800094f0,0x800094f4,0x800094f8,0x80009470,0x8000981c,0x80009828};
const unsigned int EMote2ARM_Transceiver::data_reg_rep[MAX_DATA] = {0x00180012,0x00000605,0x00000504,0x00001111,0x0fc40000,0x20046000,0x4005580c,0x40075801,0x4005d801,0x5a45d800,0x4a45d800,0x40044000,0x00106000,0x00083806,0x00093807,0x0009b804,0x000db800,0x00093802,0x00000015,0x00000002,0x0000000f,0x0000aaa0,0x01002020,0x016800fe,0x8e578248,0x000000dd,0x00000946,0x0000035a,0x00100010,0x00000515,0x00397feb,0x00180358,0x00000455,0x00000001,0x00020003,0x00040014,0x00240034,0x00440144,0x02440344,0x04440544,0x0ee7fc00,0x00000082,0x0000002a};

unsigned char ctov[16] = {0x0b,0x0b,0x0b,0x0a,0x0d,0x0d,0x0c,0x0c,0x0f,0x0e,0x0e,0x0e,0x11,0x10,0x10,0x0f};

const unsigned int EMote2ARM_Transceiver::PSMVAL[19] = {0x0000080f,0x0000080f,0x0000080f,0x0000080f,0x0000081f,0x0000081f,0x0000081f,0x0000080f,0x0000080f,0x0000080f,0x0000001f,0x0000000f,0x0000000f,0x00000816,0x0000001b,0x0000000b,0x00000802,0x00000817,0x00000003};
const unsigned int EMote2ARM_Transceiver::PAVAL[19]  = {0x000022c0,0x000022c0,0x000022c0,0x00002280,0x00002303,0x000023c0,0x00002880,0x000029f0,0x000029f0,0x000029f0,0x000029c0,0x00002bf0,0x000029f0,0x000028a0,0x00002800,0x00002ac0,0x00002880,0x00002a00,0x00002b00};
const unsigned int EMote2ARM_Transceiver::AIMVAL[19] = {0x000123a0,0x000163a0,0x0001a3a0,0x0001e3a0,0x000223a0,0x000263a0,0x0002a3a0,0x0002e3a0,0x000323a0,0x000363a0,0x0003a3a0,0x0003a3a0,0x0003e3a0,0x000423a0,0x000523a0,0x000423a0,0x0004e3a0,0x0004e3a0,0x0004e3a0};

const unsigned char EMote2ARM_Transceiver::VCODivI[16] = {0x2f,0x2f,0x2f,0x2f,0x2f,0x2f,0x2f,0x2f,0x2f,0x30,0x30,0x30,0x30,0x30,0x30,0x30};
const unsigned int EMote2ARM_Transceiver::VCODivF[16]  = {0x00355555,0x006aaaaa,0x00a00000,0x00d55555,0x010aaaaa,0x01400000,0x01755555,0x01aaaaaa,0x01e00000,0x00155555,0x004aaaaa,0x00800000,0x00b55555,0x00eaaaaa,0x01200000,0x01555555};

void EMote2ARM_Transceiver::set_event_handler(event_handler * ev_handler) {
    handler = ev_handler;
}

EMote2ARM_Transceiver::event_handler * EMote2ARM_Transceiver::get_event_handler() {
    return handler;
}

bool EMote2ARM_Transceiver::cca_measurement() {
...
void EMote2ARM_Transceiver::maca_init() {
    reset_maca();
...
void EMote2ARM_Transceiver::free_packet(volatile packet_t *p) {
...
volatile EMote2ARM_Transceiver::packet_t* EMote2ARM_Transceiver::get_free_packet() {
    volatile packet_t *p;
...
void EMote2ARM_Transceiver::post_receive() {
    ...
    if (dma_rx == 0) {
        ...
        if (dma_rx == 0) {
            db<EMote2ARM_Transceiver>(ERR) << "EMote2ARM_Transceiver::post_receive() - Trying to fill MACA_DMARX but out of packet buffers\n";
...
volatile EMote2ARM_Transceiver::packet_t* EMote2ARM_Transceiver::rx_packet() {
    volatile packet_t *p;
...
void EMote2ARM_Transceiver::post_tx() {
    while (current_action != SEQ_NOP);
    ...
    if (dma_rx == 0) {
        ...
        if (dma_rx == 0) {
            dma_rx = &dummy_ack;
            db<EMote2ARM_Transceiver>(ERR) << "EMote2ARM_Transceiver::post_tx() - Trying to fill MACA_DMARX but out of packet buffers\n";
        }
    }
...
void EMote2ARM_Transceiver::fill_packet(volatile packet_t *p, unsigned char * data, unsigned int size) {
    p->length = size;
    p->offset = 0;
...
void EMote2ARM_Transceiver::tx_packet(volatile packet_t *p) {
    IC::disable(IC::IRQ_MACA);
...
void EMote2ARM_Transceiver::free_all_packets() {
...
void EMote2ARM_Transceiver::free_tx_head() {
    volatile packet_t *p;
...
void EMote2ARM_Transceiver::add_to_rx(volatile packet_t *p) {
    IC::disable(IC::IRQ_MACA);
...
void EMote2ARM_Transceiver::maca_isr() {
...
void EMote2ARM_Transceiver::init_phy() {
...
void EMote2ARM_Transceiver::reset_maca() {
...
void EMote2ARM_Transceiver::flyback_init() {
...
void EMote2ARM_Transceiver::maca_off() {
    IC::disable(IC::IRQ_MACA);
...
void EMote2ARM_Transceiver::maca_on() {
...
unsigned char EMote2ARM_Transceiver::get_ctov(unsigned int r0, unsigned int r1) {
...
void EMote2ARM_Transceiver::radio_init() {
    volatile unsigned int i;
    // sequence 1
...
void EMote2ARM_Transceiver::set_demodulator_type(unsigned char demod) {
...
void EMote2ARM_Transceiver::set_power(unsigned char power) {
    IC::disable(IC::IRQ_MACA);
...
void EMote2ARM_Transceiver::set_channel(unsigned char chan) {
 ...
 */
unsigned int EMote2ARM_Transceiver::exec_init_entry(volatile unsigned int *entries, unsigned char *valbuf) {
    volatile unsigned int i;
...
unsigned int EMote2ARM_Transceiver::init_from_flash(unsigned int addr) {
    ...
    EMote2ARM_Flash::nvm_setsvar(0);

    err = EMote2ARM_Flash::nvm_read(EMote2ARM_Flash::gNvmInternalInterface_c, type, (unsigned char *)buf, addr, 8);
    i += 8;
    ...
    if (buf[0] == FLASH_INIT_MAGIC) {
        ...
        while (i < (len - 4)) {
            err = EMote2ARM_Flash::nvm_read(EMote2ARM_Flash::gNvmInternalInterface_c, type, (unsigned char *) buf, addr + i, 32);
 ...
 */
void EMote2ARM_Transceiver::ResumeMACASync() {
...
void EMote2ARM_Transceiver::off() {
    CPU::out32(IO::MACA_CONTROL, SEQ_NOP);
}
