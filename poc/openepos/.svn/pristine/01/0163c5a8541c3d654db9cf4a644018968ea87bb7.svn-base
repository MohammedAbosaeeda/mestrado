...
#elif defined (__softmips)

#define MACH softmips
typedef SoftMIPS		Machine;
typedef SoftMIPS_IC		IC;
typedef SoftMIPS_Timer		Timer;
typedef SoftMIPS_RTC		RTC;
typedef Dummy<0>		EEPROM;
typedef Dummy<1>		PCI;
typedef SoftMIPS_UART		UART;
typedef Dummy<2>		SPI;
/*typedef IF<Traits<SoftMIPS_Display>::on_serial,
	   Serial_Display,
	   PC_Display>::Result	Display;*/
typedef Serial_Display	Display;
typedef SoftMIPS_NIC		NIC;
typedef Dummy<4>		ADC;
typedef Dummy<5>		Temperature_Sensor;
typedef Dummy<6>		Light_Sensor;
#define __MACH_H		__HEADER_MACH(machine)
#define __MEMORY_MAP_H		__HEADER_MACH(memory_map)
#define __IC_H			__HEADER_MACH(ic)
#define __TIMER_H		__HEADER_MACH(timer)
