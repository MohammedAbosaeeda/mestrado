...
public class ArrayOptimizer {
	...
	public void addValue(int a, String b) {
		if (finalized) throw new RuntimeException("Cannot add values to finalized array.");
		values.put(new Integer(a), b);
	}
	...
	public void generateDefine() {
		SSingleton.clear();
		if (!inRAM) {
			/* This is a function, include prototype to set attribute */
			SSingleton.add(yType);
			SSingleton.add(" ");
			SSingleton.add(name);
			SSingleton.add("Values(");
			SSingleton.add(xType);
			SSingleton.add(" x) __attribute__((const));");
			SSingleton.newLine();
		}
	...
	public boolean generateArray() {
		...
		for (Iterator i = s.iterator(); i.hasNext(); ) {
			...
			if (val instanceof Integer) {
				putString = ((Integer)(val)).toString();
