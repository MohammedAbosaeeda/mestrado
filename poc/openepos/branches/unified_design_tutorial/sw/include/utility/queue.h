...
using Unified::Maybe;
#include "list.h"
using Unified::List_Element_Rank;
using Unified::List;
...
public:
    ...
    Maybe<Element*> head() { return List::head(); }
    Element * tail() { return List::tail(); }

    void insert(Element * e) { List::insert(e); }

    Maybe<Element*> remove() { return List::remove(); }
    Element * remove(Element * e) { return List::remove(e); }

    Maybe<Element*> remove(const Object_Type * obj) { return List::remove(obj); }
...
template <typename T,
	  bool atomic = false,
	  typename El = Unified::List_Elements::Doubly_Linked<T> >
...
template <typename T, 
	  ...
	  bool atomic = false,
	  typename El = Unified::List_Elements::Doubly_Linked_Ordered<T, R> >
class Ordered_Queue:
