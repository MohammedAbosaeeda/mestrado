...
void print_int_sizes() {

    // epos_prints("u32: ");
    // epos_printi(sizeof(u32_t));

    // epos_prints(" u08: ");
    // epos_printi(sizeof(u08_t));
    // epos_prints(" u16: ");
    // epos_printi(sizeof(u16_t));
}

bool_t nvmfile_init(void) {
  u16_t t = 0;
// epos_printi_hex(sizeof(nvm_header_t));
print_int_sizes();
  // epos_printi(CODESIZE);
  // epos_printi_hex(nvmfile[3]);
  nvm_header_t * nvm_header = (nvm_header_t *)nvmfile;
  u32_t features = nvmfile_read32(&nvm_header->magic_feature);
   // epos_prints("NVM_MAGIC_FEAUTURE[file] = %x\n");
   // epos_printi_hex(features);
   // epos_prints("NVM_MAGIC_FEAUTURE[vm] = %x\n");
   // epos_printi_hex(NVM_MAGIC_FEAUTURE);
  ...
  // epos_prints("t string off set: ");
  // epos_printi_hex(nvm_header->string_offset);

  t -= nvmfile_read16(&nvm_header->constant_offset);
  // epos_prints(" Constant off set: ");
  // epos_printi_hex(nvmfile_read16(&nvm_header->constant_offset));
  // epos_prints("nvmfile_constant count :");
  // epos_printi(nvmfile_constant_count);
  // epos_prints("t: ");
  // epos_printi(t);

  // epos_prints("nvmfile_constant count %i");// epos_printi(nvmfile_constant_count);
