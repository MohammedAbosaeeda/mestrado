// OpenEPOS EMote2ARM_IC Mediator Declarations

#ifndef __emote2arm_ic_h
#define __emote2arm_ic_h
...
class EMote2ARM_IC: public IC_Common {
private:
...
public:
	...
	static void int_vector(Interrupt_Id id, Interrupt_Handler handler) {
		db<EMote2ARM_IC>(INF)
				<< "EMote2ARM_IC: Registering handler for interrupt " << id
				<< " at " << (void*) handler << "\n";
		vector[id] = handler;
	}
