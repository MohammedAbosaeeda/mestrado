...
private:
    static NvmFile* _instance;
    NvmFile();
    u08_t _nvmfile_constant_count;

    static u08_t _nvmfile[];
    //u08_t  _nvmfile[];
    typedef struct {
      u08_t super;//  #define nvmfile_read                memcpy
      u08_t fields;
...
public:
   ...
   // #define nvmfile_read                memcpy
    void nvmfile_read(nvm_method_hdr_t* mhdr, nvm_method_hdr_t * mhdr_ptr,
                      int size){
        memcpy(mhdr, mhdr_ptr, size);
    }

    static NvmFile* instance();
    ...
    u08_t  nvmfile_get_method_by_class_and_id(u08_t clazz, u08_t id);
    u08_t  nvmfile_get_method_by_fixed_class_and_id(u08_t clazz, u08_t id);
};
...
#define NVMFILE_SET(a)     (void*   typedef struct {
u16_t code_index;   typedef struct {
    u16_t code_index;
    u16_t id;           // class and method id
    u08_t flags;
    u08_t args;
...
u08_t max_locals;
u08_t max_stack;
} PACKED nvm_method_hdr_t;)(((ptr_t)a) | NVMFILE_FLAG)
