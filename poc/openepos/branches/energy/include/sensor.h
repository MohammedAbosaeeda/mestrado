// OpenEPOS Sensor Mediator Common Package
...
class Audio_Sensor_Common: public Sensor_Common
{

    class Audio_Sensor_Base
    {
    public:
	Audio_Sensor_Base(unsigned int unit = 0) {}

	virtual ~Audio_Sensor_Base() {}
    ...
    template<typename Audio_Sensor, bool polymorphic>
    class Audio_Sensor_Wrapper: public Audio_Sensor_Base, private Audio_Sensor
    {
    public:

	Audio_Sensor_Wrapper(unsigned int unit = 0) : Audio_Sensor(unit) {}

	virtual ~Audio_Sensor_Wrapper() {}

	virtual int sample() {
	    return Audio_Sensor::sample();
	}

	virtual int get() {
	    return Audio_Sensor::get();
	}

	virtual bool enable() {
	    return Audio_Sensor::enable();
	}

	virtual void disable() {
	    Audio_Sensor::disable();
	}

	virtual bool data_ready() {
	    return Audio_Sensor::data_ready();
	}

    };
    template<typename Audio_Sensor>
    class Audio_Sensor_Wrapper<Audio_Sensor, false>: public Audio_Sensor
    {
    public:
	Audio_Sensor_Wrapper(unsigned int unit = 0) : Audio_Sensor(unit) {}
    };

    template<typename Audio_Sensors>
    class Meta_Audio_Sensor
    {
    private:
	static const bool polymorphic = Audio_Sensors::Polymorphic;

    public:
	typedef
	typename IF<polymorphic,
		    Audio_Sensor_Base,
		    typename Audio_Sensors::template Get<0>::Result>::Result Base;

	template<int Index>
	struct Get
	{
	    typedef
	    Audio_Sensor_Wrapper<typename Audio_Sensors::template Get<Index>::Result,
				       polymorphic> Result;
	};
    };
