...
void invert(T &n);
template<typename T>
int put_number(int fd, T num);
template<typename T>
bool add_boot_map(int fd_out, System_Info * si);
...
int main(int argc, char **argv)
{
	...
	{
		while((image_size % CONFIG.boot_length_max != 0))
	...
	if(need_si)
	{
		...
		{
			case  8:
				if(!add_boot_map<char>(fd_img, &si))
					return 1;
				break;
			case 16:
				if(!add_boot_map<short>(fd_img, &si))
					return 1;
				break;
			case 32:
				if(!add_boot_map<long>(fd_img, &si))
					return 1;
				break;
			case 64:
				if(!add_boot_map<long long>(fd_img, &si))
					return 1;
				break;
			default:
				return 1;
...
bool parse_config(FILE * cfg_file, Configuration * cfg)
{
	...
	fgets(line, 256, cfg_file);
	token = strtok(line, "=");
	if(strcmp(token, "BOOT_LENGTH_MIN"))
		return false;
	token = strtok(NULL, "\n");
	...
	fgets(line, 256, cfg_file);
	token = strtok(line, "=");
	if(strcmp(token, "BOOT_LENGTH_MAX"))
		return false;
	token = strtok(NULL, "\n");
...
//=============================================================================
template<typename T>
bool add_boot_map(int fd, System_Info * si)
{
	...
	if(!put_number(fd, static_cast<T>
	               (si->bm.img_size)))
		return false;
	if(!put_number(fd, static_cast<T>
	               (si->bm.setup_off)))
