...
void PTP::handle(){
	receive(_received_data, Traits<CMAC<Radio_Wrapper> >::MTU);
	unsigned long timestamp;
	PTP_Message::Header * receivedHeader = (PTP_Message::Header*)_received_data;
	...
	case DELAY_REQ_EVENT:{
		...
		handleDelayReq(_received_data);
		break;
	}
	case DELAY_RESP_EVENT:{
		handleDelayResp(_received_data);
		break;
	}
	}
...
int PTP::send(char * msg, int size){
	...
	/*
	while (_nic->send(NIC::BROADCAST, (NIC::Protocol) Traits<PTP>::protocolId, msg, size) != 11)
		Thread::yield();
	return 11;
	*/
}
...
//			if (_ptp_parameters._t3_delay_req_tx_time._nanoseconds != 0 &&
//					_ptp_parameters._t2_sync_rx_time._nanoseconds != 0)
				_ptp_parameters._sequence_number = req.head.sequenceId;
		}
		break;
...
void PTP::handleDelayResp(char * data)
{
	...
	//kout << "handleDelayResp" << endl;
	//kout << _ptp_parameters._sequence_number << " " <<  delayResponseMessage.head.sequenceId << endl;
...
void PTP::issueDelayResp(){
	_ptp_parameters._sequence_number++;
	//kout << "issueDelayResp " << _ptp_parameters._sequence_number << endl;
	char * msg;
