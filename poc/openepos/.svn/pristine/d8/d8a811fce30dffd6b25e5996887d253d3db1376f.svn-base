...
public:
	...
	*static_cast<int *>(sp) = swap16(Log_Addr(exit));
    ...
    static Context * restore_context(
	Log_Addr stack, unsigned int size, int (* entry)()) {
	Log_Addr fp = stack;
	Log_Addr sp = stack + size;
	sp -= sizeof(int);
	sp -= sizeof(Context);
...
public:
    static void power(char mode)
    {
        switch(mode)
        {
        case Traits<AVR8>::IDLE:
			out8(MCUCR,in8(MCUCR) & ~(SM0 | SM1 | SM2) | SE); // Idle
			break;
		case Traits<AVR8>::ADC_NOISE_REDUCTION:
			out8(MCUCR,in8(MCUCR) & ~((SM1 | SM2)));
			out8(MCUCR,in8(MCUCR) | SM0 | SE); // | SE
			break;
		case Traits<AVR8>::POWER_DOWN:
			out8(MCUCR,in8(MCUCR) & ~((SM0 | SM2)));
			out8(MCUCR,in8(MCUCR) | SM1 | SE); // | SE
			break;
		case Traits<AVR8>::POWER_SAVE:
			out8(MCUCR,in8(MCUCR) & ~(SM2) | (SM0 | SM1 | SE)); // Power_Save
			break;
		case Traits<AVR8>::NATIVE_STANDBY:
			out8(MCUCR,in8(MCUCR) & ~(SM0) | (SM1 | SM2 | SE)); // Standby
			break;
		case Traits<AVR8>::EXTENDED_STANDBY:
			out8(MCUCR,in8(MCUCR) | (SM0 | SM1 | SM2)); // | SE
			break;
        default:
            break;
        }
