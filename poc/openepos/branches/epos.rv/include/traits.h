...
// Abstractions
template <> struct Traits<Imp::Thread>: public Traits<void>
{
    ...
    static const bool enable_verification = true;
    static const bool verify_absence_of_starvation = true;
};


template <> struct Traits<Scheduler_Traits>: public Traits<void>
{
    static const bool debugged = true;
    static const bool enable_verification = true;
};
...
template <typename T> struct Traits<Imp::Scheduler<T> >: public Traits<void>
{
    static const bool debugged = true;
};
...
// template <typename Schedulable, bool enabled> struct Traits<Absence_Of_Starvation<Schedulable, enabled> >: public Traits<void>
template <> struct Traits<Absence_Of_Starvation_Traits>: public Traits<void>
{
private:
    static const unsigned long CONTEXT_SWITCHING_TIME = 0; /* us
        * This value should be determined by experimentation.
        */

    static const unsigned long NUMBER_OF_SCHEDULABLES = 1 + 1 + 5; /* The
         * quantity of
         * schedulables objects (usually threads) that you known/estimates that
         * your system would have.
         * E.g.: 1 + 1 + 5 (main, idle, plus 5 philosophers).
         */

public:
    static const bool enabled = true;
    static const bool debugged = true;
    static const bool check_starvation_of_system_threads = false; /* Weather
        * checks for the starvation of the main and idle threads or only
        * for application threads.
        */

#if 1
    static const unsigned long STARVATION_DEADLINE = NUMBER_OF_SCHEDULABLES *
            (Traits<Imp::Thread>::QUANTUM + CONTEXT_SWITCHING_TIME); /* us */
#else
    static const unsigned long STARVATION_DEADLINE = Traits<Imp::Thread>::QUANTUM; /* us */
#endif
