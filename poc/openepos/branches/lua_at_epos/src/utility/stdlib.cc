...
{
	/*
	 * n is 1 if time is odd, n is 0 if time is even.
	 * change to another approach as you wish :-)
	 */
	 
	Clock c;
	unsigned long int n = (unsigned long int)(c.now() % 2);
	return (int)Pseudo_Random::random(n);
}

void srand(unsigned int seed)
{
	Pseudo_Random::srand((unsigned long int)seed);
}
