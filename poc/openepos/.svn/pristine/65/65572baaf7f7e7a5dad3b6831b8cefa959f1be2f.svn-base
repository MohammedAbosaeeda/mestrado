...
{
    db<Setup>(TRC) << "PC_Setup::detect_pci()\n";
...
void _start()
{
	...
	*entry = 'G';
	...
	register char * dst =
	    MMU::align_page(entry + size + Traits<PC>::MAX_CPUS * sizeof(MMU::Page));
...
void setup(char * bi) 
{
    ...
    //BSP checks if all APs have started
    if(APIC::id() == 0) {
        for(unsigned int ap_number = 1; ap_number < Traits<PC>::MAX_CPUS; ap_number++) {
            if(si->bm.aps_status[ap_number] != 2) 
                db<Setup>(WRN) << "CPU (" << ap_number << ") did not start. The configuration supports ("
                               << Traits<PC>::MAX_CPUS << ") CPUs.\n";                                  
        }
    }
