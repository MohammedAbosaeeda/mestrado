...
#include "mibench_susan/include/susan_data.h"
  ...
  int            *x_size, int *y_size)
{
    ...
    *y_size = IMAGE_SIZE_Y;
    ...
    //cout << "Alloc get_image " << *x_size * *y_size << "\n";
...
void put_image(char *in,int x_size,int y_size)
{
    cout << "Image:\n{ ";
...
/* }}} */
  ...
  if ( (dt>15) && (total==0) )
  {
    cout << "Distance_thresh (" << (int)dt << ") too big for integer arithmetic.\n";
    cout << "Either reduce it to <=15 or recompile with variable \"total\"\n";
    cout << "as a float: see top \"defines\" section.\n";
    Machine::panic();
  }

  if ( (2*mask_size+1>x_size) || (2*mask_size+1>y_size) )
  {
    cout << "Mask size too big for image \n";
    Machine::panic();
  }

  //cout << "Alloc tmp_image for smoothing " << (x_size+mask_size*2) * (y_size+mask_size*2) << "\n";
  tmp_image = new uchar[ (x_size+mask_size*2) * (y_size+mask_size*2) ];
...
/* }}} */
  ...
  //cout << "Alloc dp for smoothing " << n_max*n_max << "\n";
  dp     = new unsigned char[n_max*n_max];
  dpt    = dp;
  temp   = -(dt*dt);
  ...
  for(i=-mask_size; i<=mask_size; i++)
    ...
    {
      x = (int) (100.0 * Math::exp( ((float)((i*i)+(j*j))) / temp ));
      *dpt++ = (unsigned char)x;
    }
...
static void corner_draw(uchar *in, CORNER_LIST corner_list, int x_size, int drawing_mode)
{
uchar *p;
int   n=0;
...
uchar c,*p,*cp;
  ...
  //cout << "Alloc cgx for corners " << x_size*y_size*4 << "\n";
  cgx=new int[x_size*y_size];
  //cout << "Alloc cgy for corners " << x_size*y_size*4 << "\n";
...
//SMOOTHING: finishes but result is +/-
//EDGES: ok
enum{
    MODE_SMOOTHING = 0,
    MODE_EDGES = 1,
...
/* }}} */
    case 1:
      ...
      //cout << "Alloc r for edge " << x_size * y_size*4 << "\n";
      ...
      {
        //cout << "Alloc mid for edge " << x_size * y_size << "\n";
...
/* }}} */
    case 2:
      ...
      //cout << "Alloc r for corners " << x_size * y_size*4 << "\n";
      r   = new int[x_size * y_size];
      setup_brightness_lut(&bp,bt,6);
...
/* }}} */
  ...
  //put_image((char*)in,x_size,y_size);
