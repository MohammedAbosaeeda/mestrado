...
const unsigned char AES_Common::DEFAULT_CIPHER_KEY[AES_Common::CIPHER_SIZE] = // declaration of the key
    {
        0x2b, 0x7e, 0x15, 0x16, 0x28, 0xae, 0xd2, 0xa6, 0xab, 0xf7, 0x15, 0x88, 0x09, 0xcf, 0x4f, 0x3c
    };

const unsigned char AES_Common::_sBox[256] = //Byte substitution box pre calculated
{
...
const unsigned char AES_Common::_inv_sBox[256] =
{
...
const unsigned char AES_Common::_Rcon[256] = //Round constant array pre calculated
{
...
void AES_Common::_rotate(unsigned char* in) // the variable in is the "t" variable from key_expansion funciton (array of 4 char)
{
...
unsigned char AES_Common::_expandKey(unsigned char* in)  // the variable "in" is an array (unsigned char in[EXPANDED_SIZE]). this array is composed by 11 16-byte words(keys), where
...
void AES_Common::_mixColumns(unsigned char* in) //Mix collumn operation in = state
{
...
void AES_Common::_shiftRows(unsigned char* in) // permutation step, in = state
{
...
void AES_Common::_addRoundKey(unsigned char* state, unsigned char* cipherKey, unsigned char round) // state
{
...
void AES_Common::_subBytes(unsigned char* in)//in = state, confusion step
{
...
unsigned char* AES_Common::_cipher(unsigned char in[4*NB], unsigned char* key_schedule) //in = plain text, out = encrypted text, key_schedule = round key array
{
...
void AES_Common::_invSubBytes(unsigned char* in)//in = state, confusion step
{
    for(int i=0; i<CIPHER_SIZE;i++)
    {
...
void AES_Common::_invShiftRows(unsigned char* in) // permutation step, in = state
{
    unsigned char temp[NB][NK];
    unsigned char t;
    unsigned char c,r;
...
unsigned char* AES_Common::_invCipher(unsigned char in[4*NB], unsigned char* key_schedule) //in = encrypted text, out = plain text, key schedule = round key array
{
    unsigned char* state; // state variable
    unsigned char* out; //output (not nescessary, since it uses a pointer to the inpt array array)
