...
// ---
/* MKL: instrumenting for time measurement. */
static void TimeMaster_wait_more(int m) 
{
    int i, j;
...
void vm_init(void) {
    // epos_prints("vm_init() with %d static fields\n");
    // epos_printi(nvmfile_get_static_fields());
...
void vm_new(u16_t mref) {
  ...
  if(NATIVE_ID2CLASS(mref) < NATIVE_CLASS_BASE) {
    ...
    // create object with
    h = heap_alloc(true, sizeof(nvm_word_t) * (VM_CLASS_CONST_ALLOC+nvmfile_get_class_fields(NATIVE_ID2CLASS(mref))));
...
void   vm_run(u16_t mref) {
  ...
  // // DEBUGF("Running method %d\n", mref);
    System::db<System::NanoVM>(System::TRC) << "vm_run - begin\n";
  ...
  // // DEBUGF("Allocating space for %d local(s) and %d " "stack elements - %d args\n",  mhdr.max_locals, mhdr.max_stack, mhdr.args);
  ...
  VM::getInstance()->locals(stack_get_sp() + 1);
  ...
  do {
    ...
    // // DEBUGF("%d/(sp:%d) - "DBG8" (%d): ", (pc-(u08_t*)mhdr_ptr) - mhdr.code_index,  stack_get_depth(), instr, instr);
    ...
    // move integer from stack into locals
      ...
      VM::getInstance()->local(arg0.z.bh, stack_pop()); 
      pc_inc = 2;
    ...
    // move integer from stack into locals
      ...
      VM::getInstance()->local(instr - OP_ISTORE_0, stack_pop());
    ...
    // load int from local variable (push local var)
      ...
      stack_push(VM::getInstance()->local(arg0.z.bh)); 
      pc_inc = 2;
    ...
    // push local onto stack
      ...
      stack_push(VM::getInstance()->local(instr - OP_ILOAD_0));
	...
	// // DEBUGF("iinc %d,%d\n", arg0.z.bh, arg0.z.bl); 
	VM::getInstance()->local(arg0.z.bh, (nvm_stack2int(VM::getInstance()->local(arg0.z.bh)) + arg0.z.bl) & ~NVM_IMMEDIATE_MASK); 
	pc_inc = 3;
	...
	// make space for locals on the stack
	// DEBUGF("Return from method with %d local(s) and %d " "stack elements - %d args\n", 		   mhdr.max_locals, mhdr.max_stack, mhdr.args);
	...
	VM::getInstance()->locals(stack_get_sp() - old_localsoffset);
	...
	tmp2 = stack_get_sp() - VM::getInstance()->locals();
...
#if 0 // For time measurement XXX mkl
            while(1) {
                TimeMaster_wait_more(100000);  
                
                
                turn_on_led1();   
                for (int indesus = 0; indesus < 10000; indesus ++) {
#endif // For time measurement XXX mkl
                    native_invoke(arg0.w);
                    pc_inc = 3;   // prefetched data used
#if 0 // For time measurement XXX mkl      
                }
                turn_off_led1();
                cout << "Native method executed OK\n";
                stack_pop(); // Simulating consumming value from int NIC::send
            }
#endif // For time measurement XXX mkl
        }
    }
...
# endif
    ...
    // move integer from stack into locals
      ...
      VM::getInstance()->local(instr - OP_FSTORE_0, stack_pop());
    ...
    // push local onto stack
      ...
      stack_push(VM::getInstance()->local(instr - OP_FLOAD_0));
  ...
  heap_unsteal(sizeof(nvm_stack_t) * (mhdr.max_locals + mhdr.max_stack + mhdr.args));
    ...
    System::db<System::NanoVM>(System::TRC) << "vm_run - end\n";
}
