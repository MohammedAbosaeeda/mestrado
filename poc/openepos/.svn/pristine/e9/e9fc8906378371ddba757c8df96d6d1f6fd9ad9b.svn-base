...
public:
    ...
    Element * search(const Object_Type * obj) {
        ...
        for(; e && (e->object() != obj); e = e->next());
        /*for(; e && (e->object() != obj); e = e->next()) {
            kout << "List::search() e->obj() = " << (void *) e->object() << " obj = " 
                 << (void *) obj << "\n";
        }
        if(!e) {
          kout << "List::search() return e = NULL\n";
        } else {
          kout << "List::search() return e != NULL\n";
        }*/
        return e;
...
public:
    ...
    /*void print() {
        for(unsigned int i = 0; i < R::HEADS; i++) {
            kout << "_chosen[" << i << "] = " << (void *) _chosen[i] << "\n";
        }
    }*/
    
    Element * volatile & chosen(int i) { return _chosen[i]; }
    ...
    void update_lowest_priority_running(void) {
        for(unsigned int  i = 0; i < R::HEADS; i++) {
            if(_chosen[i] == 0) {
              ...
              break;
            } else if(_chosen[i]->rank() > _chosen[_lowest_priority_running]->rank()) {
                    _lowest_priority_running = i;
            }
        }
    }
...
private:
    volatile unsigned int _lowest_priority_running;
...
public:
    ...
    /*void insert_merging_debug(Element * e, Element ** m1, Element ** m2) {
        db<Lists>(TRC) << "Grouping_List::insert_merging(e=" << e 
            << ")    \n";
        _grouped_size += e->size();
        *m1 = *m2 = 0;
        
        //kout << "Grouping_List::insert_merging() pdp=" << (void *) CPU::pdp() << " sp=" << (void *) CPU::sp() << " eip=" << (void *) CPU::eip()
        //<< " eax=" << (void *) CPU::eax() << " cs=" << (void *) CPU::cs() << " ds=" << (void *) CPU::ds() 
        //<< " es=" << (void *) CPU::es() << " ss=" << (void *) CPU::ss()
        //<< " fs=" << (void *) CPU::fs() << " gs=" << (void *) CPU::gs() << "\n";
        
        kout << "Grouping_List::insert_merging() search.. e = " << (void *) e << " e->size() = " << (void *) e->size() 
        << " e->object() " << (void *) e->object() << " obj + size = " 
        << (void *)(e->object() + e->size()) << "\n";
        
        Element * r = search(e->object() + e->size());
        kout << "Grouping_List::insert_merging() search has returned r = " << (void *) r << "\n";
        
        kout << "Grouping_List::insert_merging() search_left..";
        Element * l = search_left(e->object());
        kout << " l = " << (void *) l << "\n";
        
        if(!l) {
            kout << "Grouping_List::insert_merging() insert_tail() e=" << (void *) e << "\n";
            insert_tail(e);
        }
        if(r) {
            e->size(e->size() + r->size());
            remove(r);
            *m1 = r;
            kout << "Grouping_List::insert_merging() if(r) new e->size() = " 
                << (void *) e->size() << " *m1=r=" << (void *) *m1 << "\n";
        }
        if(l) {
            l->size(l->size() + e->size());
            *m2 = e;
            kout << "Grouping_List::insert_merging() if(l) new l->size() = " 
                 << (void *) l->size() << " *m2=e=" << (void *) *m2 << "\n";
        }
        kout << "Grouping_List::insert_merging() m1=" << (void *) *m1 << " m2=" << (void *) *m2 << "\n";
        kout << "Grouping_List::insert_merging() head = " << (void *) head() << " head size = " 
        << head()->size() << " next size = " << ((head()->next() != 0) ? head()->next()->size() : 0) 
        << " grouped_size() = " << _grouped_size << " size() = " << size() << "\n";
    }
    
    Element * search_size_debug(unsigned int s) {
        kout << "**Grouping_List::search_size_debug() initializing\n";
        Element * e = head();
        for(; e && (e->size() < sizeof(Element) + s) && (e->size() != s); e = e->next());
        
        if(!e) {
            kout << "**Grouping_List::search_size() ERROR\n";
            e = head();
            int i = 0;
            for(; e && (e->size() < sizeof(Element) + s) && (e->size() != s); e = e->next()) {
                kout << "[" << i++ << "] e->size() = " << e->size() << " sizeof(Element) + s = " 
                << sizeof(Element) + s << " grouped_size = " << _grouped_size << " size() = " << size() << "\n";
            }
            
        } else {
            kout << "**Grouping_List::search_size() return e size = " << e->size()
            << " e->head() size = " << head()->size() << " grouped_size() = " << _grouped_size 
            << " size() = " << size() << "\n";
        }
    ...
    Element * search_decrementing_debug(unsigned int s) {
        db<Lists>(TRC) << "Grouping_List::search_decrementing(s=" 
            << s << ")\n";
        kout << "Grouping_List::search_decrementing(s=" << s << ")\n";

        Element * e = search_size_debug(s);
        if(e) {
            e->shrink(s);
            _grouped_size -= s;
            if(!e->size())
                remove(e);
            kout << "Grouping_List::search_decrementing(s=" << s << ") e size = " << e->size()
            << " e->head() size = " << head()->size() << " grouped_size = " << _grouped_size << "\n";
        } else 
          kout << "!!!Grouping_List::search_decrementing ERROR list size = " <<
          _grouped_size << " requested size = " << s << " head size = " << head()->size() << "\n";
