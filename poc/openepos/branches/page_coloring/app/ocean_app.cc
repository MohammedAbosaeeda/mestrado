...
#include "ocean/epos/pthread.h"
#include "ocean/epos/epos_wrappers.h"
#include <utility/malloc.h>
#include <clock.h>
#include <alarm.h>
#include "ocean/epos/math.h"
#include <display.h>
#include <chronometer.h>
#define MAX_THREADS 32
pthread_t *PThreadTable[MAX_THREADS];
System::Display d;
System::Chronometer chron;


//#define DEFAULT_N      258      // size of grid -n
#define DEFAULT_N      130
#define DEFAULT_P        4      // number of processors -p
#define DEFAULT_E        1e-7   // tolerance -r
//#define DEFAULT_T    28800.0  // timestamp (seconds) -t
#define DEFAULT_T    1000.0
#define DEFAULT_R    20000.0    // distance between grid points in meters -r
//#define DEFAULT_R    2000.0
#define UP               0
#define DOWN             1
#define LEFT             2
#define RIGHT            3
...
int main(int argc, char *argv[])
{
   ...
   printf("\n");*/
   cout << "\n";
   cout << "Ocean simulation with W-cycle multigrid solver\n";
   cout << "    Processors                         : " << nprocs << endl;
   cout << "    Grid size                          : " <<  im << " x " << jm << endl;
   cout << "    Grid resolution (meters)           : " << (float) res << endl;
   cout << "    Time between relaxations (seconds) : " << (float) dtau << endl;
   cout << "    Error tolerance                    : " << (float) tolerance << endl;
   cout << "\n";
   ...
   if (xprocs == 0) {
     ...
     cout << "Could not find factors for subblocking\n";
     exit(-1);
   }
   ...
   if (numlev == 0) {
     ...
     cout << "Must have at least 2 grid points per processor in each dimension\n";
     exit(-1);
   }
...
#else
   {
    bars->barrier = (pthread_barrier_t *) malloc(sizeof(pthread_barrier_t));
...
/* initialize constants and variables
...

   {
	...
	for (i = 0; i < (nprocs); i++) {
		...
		//Error = pthread_create(PThreadTable[i], NULL, (int (*)(void *))(slave), NULL);
		if((i % 2) == 0)
          shared_data = true;
		PThreadTable[i] = new System::Thread(slave, System::Thread::READY, System::Thread::NORMAL, System::Traits<System::Machine>::APPLICATION_STACK_SIZE, shared_data);
		if (Error != 0) {
...
};
   {
    ...
    (computeend) = (unsigned long) clock.now();
   ...
   cout << "                  Total          Multigrid         Multigrid\n";
   cout << " Proc             Time             Time            Fraction\n";
   cout <<  "    0         " << (float) gp[0].total_time << "         " << (float) gp[0].multi_time << "          " << (float) (gp[0].multi_time/gp[0].total_time) << "\n";
   ...
   if (do_stats) {
     ...
     for (i=1;i<nprocs;i++) {
       ...
       cout << "  " <<  i << "   " << (float) gp[i].total_time << "   " <<  (float) gp[i].multi_time << "   " << (float) (gp[i].multi_time/gp[i].total_time) << endl;
     }
     ...
     cout << "  Avg   " << (float) avg_total << "    " << (float) avg_multi << "        " << (float) avg_frac << "\n";
     cout << "  Min   " << (float) min_total << "    " << (float) min_multi << "        " << (float) min_frac << "\n";
     cout << "  Max   " << (float) max_total << "    " << (float) max_multi << "        " << (float) max_frac << "\n";
   }
   //printf("\n");
   cout << "\n";
   ...
   cout << "Start time                        : " << global->starttime << "\n";
   cout << "Initialization finish time        : " << global->trackstart << "\n";
   cout << "Overall finish time               : " << computeend << "\n";
   cout << "Total time with initialization    : " << computeend-global->starttime << "\n";
   cout << "Total time without initialization : " << computeend-global->trackstart << "\n";
   cout << "    (excludes first timestep)\n";
   cout << "\n";
   
   cout << "Chronometer time = " << (time_end / 1000000) << " sec\n";
