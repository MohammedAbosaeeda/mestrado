...
Thread::Scheduler Thread::_scheduler;
...
//grw Troquei de running para choose
...
void Thread::ready_enqueue() 
{
    prevent_scheduling();

    db<Thread>(TRC) << "Thread::ready_enqueue(this=" << this << ")\n";
    if(_state == SUSPENDED) {
	_state = READY;
	_scheduler.resume(this);
    } else
	db<Thread>(WRN) << "Ready_enqueue called for unsuspended object!\n";
    
    //allow_scheduling();   // alarm will allow
}
...
void Thread::sleep(Queue * q)
{
    ...
    Alarm::reset_master();
//grw Troquei de chosen para choose
    switch_threads(thr, _scheduler.choose());
...
//TODO    Temperature_Sensor sensor; // = new Temperature_Sensor();
