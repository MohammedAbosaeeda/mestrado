...
void native_java_io_printstream_invoke(u08_t mref, StackVm* stack) {
  char tmp[8];

  if(mref == NATIVE_METHOD_PRINTLN_STR) {

    native_print( ((char*)stack->stack_pop_addr()), true);
  ...
  stack->stack_pop(); // pop output stream reference
}
...
void native_java_lang_stringbuffer_invoke(u08_t mref) {
  if(mref == NATIVE_METHOD_INIT) {
    ...
    *(char*)stack_pop_addr() = 0;
    ...
    src = (char*) stack_peek_addr(0);
    dst = (char*) heap_get_addr(stack_peek(1) & ~NVM_TYPE_MASK);
