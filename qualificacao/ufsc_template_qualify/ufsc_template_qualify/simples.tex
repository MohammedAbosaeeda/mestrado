\chapter{Introdução}
\section{Introdução}
A consolidação da internete como ferramenta de comunicação abrangente à todas as áreas do conhecimento propicia um  ambiente para o desenvolvimento de tecnologias e aplicações inovadoras. A distribuição deste ambiente acarreta em heterogeneidade, variabilidade e irregularidade na comunicação e falta de confiabilidade. Já é existente e está em crescimento a necessidade de distribuir em larga escala a computação. Estas evoluções tecnológicas estimulam a criação de modelos e paradigmas computacionais, seja através de inovação ideológica ou edificação do conhecimento já adquirido. 

Dentre as soluções propostas nesta evolução é comum o uso da abordagem de troca de mensagens para a comunicação das componentes distribuídas, entretanto através desta estratégia pode haver, e em geral há, perda da confiabilidade e do anonimato. Em função disto, se fazem necessárias inovações na área. Além disto, o uso irrestrito de recursos é um problema não apenas do ponto de vista financeiro, mas também se torna um vilão no fator confidencialidade, dado que, quanto mais distribuída, ou difundida é uma informação, mais simples se torna a interceptação da mesma por terceiros.

Outro aspecto à ser analisado é a segurança do funcionamento, afinal é preciso se que se garanta o funcionamento correto de um sistema distribuído, ainda que este sistema esteja sob ataques ou que suas componentes estejam falhas. Pouco importa a natureza da falha, podem ser intencionais ou não e maliciosas ou não.

A tolerância a faltas e falhas é,  no contexto de distribuição de sistemas, um assunto de suma importância e deve ser visto como um requisito de qualidade.
...
\textbf{Sistemas Bizantinos de quórum}
...
\textbf{Espaço de Tuplas Bizantinas Linearizáveis}



\textbf{Fundamentação do projeto e novas técnicas}



A filosofia da ETBL é usar protocolos baseados em quórum para as operações de leitura e escrita e uma primitiva de acordo para as operações de leitura-remoção.
...
\textbf{Inserção de Tupla - Protocolo de saída}
...
\textbf{Leitura Destrutiva de Tupla}
...
\textbf{Espaço de Tuplas com Segurança de Funcionamento}
...
\textbf{Políticas de Segurança de Granularidade Fina}
...
\textbf{Espaço aumentado de tuplas e Protegido por Políticas}
...
\textbf{A Arquitetura do RePEATS}
...
As seguintes propriedades tem que ser satisfeitas para que o RePEATS esteja sempre vivo e seguro:
\begin{itemize}
...
\textbf{Algoritmo de Ordenação de Requisições}
...
\textbf{Mecanismo de \textit{Logging}, \textit{Checkpoint} e Recuperação de Réplicas}
...
\textbf{Objetos de Política Forçada}
...
\textbf{Consenso Forte Multivalorado}
...
\textbf{Consenso Multivalorado Padrão}
...
\chapter{Conceitos Básicos em Computação Distribuída}
...
\chapter{Tolerância a Faltas Bizantinas}
