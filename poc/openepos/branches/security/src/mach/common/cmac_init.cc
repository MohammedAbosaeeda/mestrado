...
template<> volatile bool CMAC<Radio_Wrapper>::_state_machine_lock = false;
template<> volatile bool CMAC<Radio_Wrapper>::_send_lock = false;
template<> volatile bool CMAC<Radio_Wrapper>::_receive_lock = false;
...
void CMAC<Radio_Wrapper>::init(unsigned int n) {
    ...
    if (Traits<CMAC<Radio_Wrapper> >::time_triggered) {
		...
		// so this prevents the IDLE thread from ever being scheduled.
			...
			Thread * t = new (kmalloc(sizeof(Thread)))Thread(&workaround_for_time_triggered_cmac);//, Thread::READY, Thread::NORMAL, 1800);
			Thread * tt = new (kmalloc(sizeof(Thread)))Thread(&workaround_for_time_triggered_cmac);//, Thread::READY, Thread::NORMAL, 1800);
		/*******************************************************************************************/
    }
