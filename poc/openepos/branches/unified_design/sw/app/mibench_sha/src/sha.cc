...
void sha_stream(SHA_INFO *sha_info, const char *file_data, int data_size)
{
    int i;
    ...
    int file_idx = 0;
    while(file_idx < data_size) {
        int i = 0;
        for (; (i < BLOCK_SIZE) && (file_idx < data_size); ++i, ++file_idx) {
            data[i] = file_data[file_idx];
        }
...
{
    System::OStream ostream;

    ostream << (void*) sha_info->digest[0] << " " <<
               (void*) sha_info->digest[1] << " " <<
               (void*) sha_info->digest[2] << " " <<
               (void*) sha_info->digest[3] << " " <<
               (void*) sha_info->digest[4] << "\n";
