...
#include <mach/axi4lite/noc.h>


__BEGIN_SYS

AXI4LITE_NOC* AXI4LITE_NOC::_instantance = 0;

AXI4LITE_NOC::AXI4LITE_NOC():NOC_Common(){//,_lock_sem(1){
    ...
    if(_info.data_width != Traits<AXI4LITE_NOC>::NOC_DATA_BUS_SIZE){
        db<AXI4LITE_NOC>(WRN) << "AXI4LITE_NOC(): Data bus size = " << _info.data_width << " differs from Traits\n";
    }
...
void AXI4LITE_NOC::send(Packet const* _data){
    while(wait());
    for (unsigned int i = 0; i < Traits<AXI4LITE_NOC>::PROC_NOC_BUS_RATE; ++i) data(i,_data->data[i]);
    wr();
}
void AXI4LITE_NOC::send(Address const* address, Packet const* _data){
    send_header(address);
    send(_data);
}
...
void AXI4LITE_NOC::receive(Packet* _data){
    while(!nd());
    for (unsigned int i = 0; i < Traits<AXI4LITE_NOC>::PROC_NOC_BUS_RATE; ++i) _data->data[i] = data(i);
    rd();
}
void AXI4LITE_NOC::receive(Address* address, Packet* _data){
