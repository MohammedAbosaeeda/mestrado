...
void collect_exec_time(int);
void print_all_exec_time();
...
TSC::Time_Stamp total_exec_time[TEST_REPETITIONS];
TSC::Time_Stamp thread_exec_time[THREADS][ITERATIONS];
TSC::Time_Stamp thread_wcet[TEST_REPETITIONS];
TSC::Time_Stamp thread_mean[TEST_REPETITIONS];
TSC::Time_Stamp thread_var[TEST_REPETITIONS];
...
int run(int test)
{
    ...
    total_exec_time[test] = chrono.read();
    ...
    collect_exec_time(test);
    
    //print_alarm();
    //print_alarm();
        
    cout << "GEDF test (" << test << ") done in "
         << chrono.read() / 1000000 << " seconds exec = " 
        << total_exec_time[test] << "\n";
...
{
    TSC::Time_Stamp wcet = 0;
    TSC::Time_Stamp m = mean_wo_mul(total_exec_time, TEST_REPETITIONS, &wcet);
    TSC::Time_Stamp var = variance(total_exec_time, TEST_REPETITIONS, m);
...
{
    TSC::Time_Stamp tmp;
    TSC::Time_Stamp highest = find_highest(thread_wcet, TEST_REPETITIONS);
    TSC::Time_Stamp m = mean_wo_mul(thread_mean, TEST_REPETITIONS, &tmp);
    TSC::Time_Stamp v = mean_wo_mul(thread_var, TEST_REPETITIONS, &tmp);
    
    cout << "Final WCET = " << highest << " MEAN = " 
        << m << " VAR = " <<  v << "\n";
}

void collect_exec_time(int test)
{
    TSC::Time_Stamp w = 0;
    TSC::Time_Stamp m = 0;
    TSC::Time_Stamp var = 0;
    
    for(int i = 0; i < THREADS; i++) {
        TSC::Time_Stamp tmp_w;
        TSC::Time_Stamp tmp = mean_wo_mul((unsigned long long *) &thread_exec_time[i], ITERATIONS, &tmp_w);
        ...
        tmp = variance((unsigned long long *) &thread_exec_time[i], ITERATIONS, tmp);
...
int job(unsigned int exec_time, int id)
{
    ...
    for(int i = 0; i <  ITERATIONS; i++) {
      ...
      thread_exec_time[id][i] = end - start;
      
      //c.stop();
      //cout << "loop time = " << (c.read() * 1000000) << endl;
      //s.p();
      //cout << " (Thread " << id << " end)\n";
      //s.v();
    }
    //s.p();
    //cout << "thread " << (threads_parameters[id][0] >> 8) << 
    //     " exec tsc " << max << " END \n";
    //s.v();
    return sum;
}
