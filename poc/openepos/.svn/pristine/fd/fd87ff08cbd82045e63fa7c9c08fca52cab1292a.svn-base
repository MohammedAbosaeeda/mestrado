...
class Zynq_UART : public UART_Common
{
    ...
    void init(){
        ...
        /*	Configure UART_Ref_Clk (assuming I/O PPL = 667mhz.
			Dividing it by 20 to get 33mhz), based on p. 628.*/
		write(SLCR::UART_CLK_CTRL, SLCR::DIVISOR, 20); //decimal
...
#endif
		/*
		write_bit(MODE_REG0, 0xffff,0x20);
		write_bit(MODE_REG0, CLKS, 0);
		write_bit(MODE_REG0, CHRL, 0);
		write_bit(MODE_REG0, PAR, 4);
		write_bit(MODE_REG0, NBSTOP, 0);
		write_bit(MODE_REG0, CHMODE, 0);
		*/
		...
		write_bit(BAUD_RATE_DIVIDER_REG0, CD, asdf);
