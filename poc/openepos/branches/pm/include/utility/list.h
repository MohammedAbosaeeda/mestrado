...
namespace List_Elements
{
	...
	const R & rank() const { return _rank; }
	void rank(const R & r) { _rank = r; }
//grw	void rank(const R & r) { kout << "Doubly_Linked_Scheduling\n"; _rank = r; }
	int promote(const R & n = 1) { /*kout << "Doubly_Linked_Scheduling::promote("<< n << ") =" << _rank <<"\n";*/_rank -= n; return _rank; }
	int demote(const R & n = 1) { _rank += n; return _rank; }
...
// Doubly-Linked, Relative Scheduling List
template <typename T,
	  typename R , 
	  typename El >
class Scheduling_List<T, R, El, true>: private Ordered_List<T, R, El, true>
{
private:
    typedef Ordered_List<T, R, El, true> Base;
