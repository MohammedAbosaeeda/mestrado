...
private:
    Alarm(const Microsecond & time, Handler * handler, int times, bool int_enable);

    static Microsecond period()
	{
		//db<Alarm>(TRC) << "Alarm::period!\n"; //by david
		//_timer.frequency(FREQUENCY);
		//db<Alarm>(TRC) << "Alarm::frequency => " << frequency() << "\n";
		//db<Alarm>(TRC) << "Alarm::Nao deu pau...\n";
