...
#include "list.h"
using Unified::Simple_Ordered_List;
#include "vector.h"
...
public:
    ...
    typedef typename Unified::List_Elements::Singly_Linked_Ordered<T, Key> Element;
...
public:
	...
	if(e)
	    return e;
	return _synonyms.remove(obj).get();
    }
	...
	if(e && (e->key() == key))
	    return e;
	return _synonyms.search_rank(key).get();
    }
	...
	return _synonyms.remove_rank(key).get();
    }
...
template <typename T,
         unsigned int SIZE,
         typename Key = int,
         typename El = Unified::List_Elements::Singly_Linked_Ordered<T, Key>,
...
public:
	...
	for(int i = 0; i < SIZE; i++) {
	    Element * e = _table[i].remove(obj).get();
	    if(e)
		return e;
	}
	return 0;
    ...
    Element * search_key(const Key & key) {
	return _table[key % SIZE].search_rank(key).get();
    }
    
    Element * remove_key(int key) {
	return _table[key % SIZE].remove_rank(key).get();
    }
