...
public:
    Periodic_Thread(int (* entry)(), 
            ...
            Criterion criterion = Criterion::PERIODIC,
	...
	: Thread(entry, BEGINNING, period, stack_size),
	  _semaphore(0),
	  _handler(&_semaphore),
	  _alarm(period, &_handler, times, criterion.alarm_cpu_queue())
    {
      //kout << "Periodic_Thread()\n";
    ...
    {
        //kout << "crit.queue = " << criterion.queue() << " deadline = " 
        //    << criterion.get_deadline() << " priority = " << 
        //    criterion.get_priority() << "\n";
    if((state == READY) || (state == RUNNING)) {
        _state = SUSPENDED;
        resume();
    } else
    ...
    static void wait_next() {
      //kout << "Periodic_Thread wait_next()\n";
