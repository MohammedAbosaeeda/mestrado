...
private:
    typedef
    Unified::List_Elements::Singly_Linked_Ordered<Conditional_Observer> Element;
...
public:
    ...
    typedef Unified::List_Elements::Singly_Linked_Ordered<Observer> Element;
    ...
    virtual void notify(T src,T dst,long c,void * data,unsigned int size) {
        ...
        Maybe<Element*> _e = _observers.head();
        if(_e.exists()) {
            for(Element * e = _e.get(); e; e = e->next()) {
                if(e->rank() == c) {
                    db<Observed>(INF) << "Observed::notify(this=" << this << ",obs=" << e->object() << ")\n";
