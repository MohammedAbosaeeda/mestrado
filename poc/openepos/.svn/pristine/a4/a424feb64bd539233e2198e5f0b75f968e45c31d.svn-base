// OpenEPOS EMote2ARM_Transceiver Mediator Declarations

#ifndef __emote2arm_transceiver_h
#define __emote2arm_transceiver_h

#include <system/config.h>

__BEGIN_SYS

class EMote2ARM_Transceiver {
private:
	enum Register_Bits {
		// MACA_CONTROL
		CONTROL_ISM = 20,
	...
	enum {
		MODE_NO_CCA, MODE_NON_SLOTTED_CSMA_CA, MODE_SLOTTED_CSMA_CA,
	};
...
public:
	EMote2ARM_Transceiver() {
	}
	~EMote2ARM_Transceiver() {
	}

	enum Event {
		SFD_DETECTED = 0, FRAME_END
	};
	...
	void fill_packet(volatile packet_t * p, unsigned char * data,
			unsigned int size);
	...
	static unsigned int exec_init_entry(volatile unsigned int *entries,
