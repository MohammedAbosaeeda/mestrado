...
% + Presents ME, remember it is widely used for video encoding, including H.264.
Motion Estimation (ME) is a technique used during video compression to explore temporal
...
such similar regions across neighbor frames, thus allowing for difference-based 
encoding.
...
\emph{Block-Matching Algorithms} (BMAs).
...
Furthermore, the wide range of computer architectures available for the deployment of video encoders
spans a myriad of specific optimized implementations of encoding algorithms
...
decoding components allows the use of different optimized versions of algorithms (e.g. ME) 
without the need of modifying the whole encoder/decoder design, while maintaining compatibility
...
%  (parallel algorithms).
%  ...
In this paper, we show how a careful design process can be used 
...
using three distinct computer architectures: the Cell Broadband Engine (Cell BE), an Intel IA32 
...
Section~\ref{impl} describes the implementation of DMEC;
Section~\ref{eval} evaluates DMEC both in isolation and integrated with the standard
H.264 encoder;
