...
\item $ {\langle REPLY, t, c, r_i, r \rangle_\sigma}_s $: Mensagem gerada e enviada pelos servidores como reposta a uma requisição de um cliente. É composta pelo \textit{timestamp} $t$ da requisição, a assinatura $c$ do cliente, a assinatura $r_i$ da réplica que gerou a mensagem e o resultado $r$ da execução da operação requisitada;
\item $ {\langle PROPOSE, n, m \rangle_\sigma}_s $: Mensagem gerada e escrita no registrador pela réplica primária. Possui em seu corpo a mensagem $m$ do cliente e o número de sequência $n$ gerado para a mensagem;
\item $ {\langle ACCEPT, n, m \rangle_\sigma}_s $: Mensagem gerada e escrita no registrador por todas as réplicas que aceitarem uma proposição da réplica primária. Contém os mesmos atributos de uma mensagem PROPOSE, isto é, a mensagem $m$ do cliente com seu respectivo número de sequência $n$.
\item $ {\langle ELECTION \rangle_\sigma}_s $: Mensagem gerada e escrita no registrador por todas as réplicas que suspeitam da réplica primária.
\end{itemize}
...
\section{Algoritmo}
Esta seção provê uma descrição mais profunda do algoritmo Paxos modificado. São apresentadas as sequências de operações efetuadas, em caso normal de operação e em casos de presença de faltas.

\subsection{Operação em Caso Normal}
\begin{enumerate}
\item O cliente envia uma requisição para todos os servidores. O cliente $c$, portanto, envia uma mensagem $ {\langle REQUEST, o, t, c \rangle_\sigma}_c $ através de uma requisição demonstrando interesse na execução de uma operação. O campo $t$ é a estampilha temporal da requisição que é utilizada para assegurar a semântica de execução única. Isto é, os servidores não executam uma requisição para um cliente com um $t$ menor que o último executado para o mesmo cliente, evitando assim que uma mesma requisição seja executada duas ou inúmeras vezes.

\item O servidor primário, ao receber a requisição do cliente, gera uma proposta de ordenação para a mensagem e a escreve no registrador em seu espaço reservado. A mensagem de proposta possui o formato $ {\langle PROPOSE, n, m \rangle_\sigma}_p $, onde $m$ representa a mensagem e $n$ o número de ordenação da mesma. Após a escrita, o nodo primário aguarda para que pelo menos uma réplica responda sua proposta com uma mensagem de aceitação. Se isto ocorrer, a réplica primária entenderá que sua mensagem proposta é válida e, portanto, estará apta à executar a operação do cliente e em seguida enviar a resposta no formato $ {\langle REPLY, t, c, r_i, r \rangle_\sigma}_i $. Este comportamento pode ser observado no algoritmo \ref{alg:NP}.

\begin{algorithm}[H]
\caption{Paxos com registradores}
\label{alg:NP}
\textbf{Nodo Primário}
...
f : int // Maximum faults tolerated

\textbf{Variables:}

sn : set of nodes

am  : set of acceptance messages for a proposal

req : request message

proposal : an ordering proposal

rto : boolean that denotes if the node reached the timeout

INIT\;

req = receiveMessage()\;

proposal = generateOrderingNumber( req )\;

write( proposal )\;

  \While{am $< 1$ OR NOT rto}{

     am = getAcceptanceFor( req, sn )\;
...
\item As réplicas primárias, ao receber a requisição do cliente, aguardam pela proposta de ordenação do servidor primário. Ao receberem a proposta do líder, as réplicas a validam, i.e., (i) a assinatura de $m$ é verificada e dada como correta e (ii) nenhuma mensagem PROPOSE tenha sido anteriormente aceita para o número de sequência $i$ ou se no mínimo $f + 1$ mensagens de aceitação forem recebidas para a mesma mensagem. Se a réplica $i$ aceitar a mensagem, então a mesma irá escrever no registrador uma mensagem $ {\langle ACCEPT, n, m \rangle_\sigma}_a $ contendo os mesmos atributos da mensagem PROPOSE, isto é, a mensagem $m$ do cliente com seu respectivo número de sequência $n$. Ao aceitar a proposta do líder, a réplica $i$ estará apta à executar a operação do cliente e em seguida enviar a resposta no formato $ {\langle REPLY, t, c, r_i, r \rangle_\sigma}_i $. Este comportamento pode ser observado no algoritmo \ref{alg:NNP}.

\item Após receber pelo menos $f + 1$ respostas iguais entre si e de servidores diferentes, o cliente as aceita a mensagem.
\end{enumerate}
...
proposal = waitForProposal()\;

  \eIf{ isAcceptable(proposal, req) OR  isAccepted(req, sn) }{
   writeAcceptanceMessage(proposal)\;
   executeAndReply(req)\;
   return\;
   }{
   askForLeaderElection()\;
   return\;
  }
...
\subsection{Operação em Casos de Presença de Faltas}
Os servidores replicados não pode ficar indefinidamente aguardando pelas mensagens da réplica primária, pois , a mesma pode estar faltosa. Portanto, sempre que uma mensagem é recebida, cada réplica inicia um temporizador que finaliza em um tempo $T_p$$_r$$_o$$_p$, no intuito de aguardar por uma proposta do líder. 
O estado de presença de faltas é atingido (i) quando o temporizador da réplica chega ao final da contagem sem que se tenha se tenha recebido uma mensagem de proposta do líder, (ii) se a réplica $i$ receber $f + 1$ mensagens para troca de líder $ {\langle ELECTION \rangle_\sigma}_a $ ou se a réplica não aceitarem como válida a proposta do líder. A réplica permanece neste estado até receber uma mensagem PROPOSE do novo líder.
