...
int PCNet32::init(unsigned int unit, System_Info * si)
{
    ...
    PCI::header(loc, &hdr);
    if(!hdr) {
	db<PCNet32>(WRN) << "PCNet32::init: PCI header failed!\n";
	return 1;
    }
    db<PCNet32>(INF) << "PCNet32::init: PCI header=" << hdr << "}\n";
    if(!(hdr.command & PC_PCI::COMMAND_IO))
	db<PCNet32>(WRN) << "PCNet32::init: I/O unaccessible!\n";
    if(!(hdr.command & PC_PCI::COMMAND_MASTER))
	db<PCNet32>(WRN) << "PCNet32::init: not master capable!\n";

    // Get I/O base port
    IO_Port io_port = hdr.region[PCI_REG_IO].phy_addr;
    db<PCNet32>(INF) << "PCNet32::init: I/O port at " 
		     << (void *)(int)io_port << "\n";

    // Get I/O irq
    IO_Irq irq = hdr.interrupt_line;
    db<PCNet32>(INF) << "PCNet32::init: PCI interrut pin "
