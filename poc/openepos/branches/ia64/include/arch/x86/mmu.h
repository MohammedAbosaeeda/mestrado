// EPOS CPU MMU Mediator Declarations
...
class CPU_MMU: public MMU_Common<9, 9, 9, 9, 12>
{
private:
...
public:
    ...
    public:
	CPU_Flags() {}
	CPU_Flags(const CPU_Flags & f) : _flags(f._flags) {}
	CPU_Flags(unsigned int f) : _flags(f) {}
	CPU_Flags(Flags f) : _flags(PRE | ACC |
				     ((f & Flags::RW)  ? RW  : 0) |
				     ((f & Flags::USR) ? USR : 0) |
				     ((f & Flags::CWT) ? PWT : 0) |
				     ((f & Flags::CD)  ? PCD : 0) |
	...
	friend Debug & operator << (Debug & db, CPU_Flags f)
	...
	void map(int from, int to, CPU_Flags flags) {
	...
	void map_contiguous(int from, int to, CPU_Flags flags) {
	    remap(alloc(to - from), from, to, flags);
	}
	void remap(Phy_Addr addr, int from, int to, CPU_Flags flags) {
    ...
    public:
	    ...
	    : _from(0), _to(pages(bytes)), _pts(page_tables(_to - _from)),
	      _flags(CPU_Flags(flags)), _pt(calloc(_pts)) {
	    if(flags & CPU_Flags::CT)
	...
	~Chunk() {
	    if(!(_flags & CPU_Flags::IO)) {
		if(_flags & CPU_Flags::CT)
	...
	void activate() { CPU::pdp(reinterpret_cast<CPU::Reg64>(_pd)); }
	...
	Log_Addr attach(const Chunk & chunk, Log_Addr addr) {
 	...
 	void detach(const Chunk & chunk) {
	    ...
	    db<CPU_MMU>(WRN) << "CPU_MMU::Directory::detach(pt="
			      << chunk.pt() << ") failed!\n";
 	}
	...
	bool attach(unsigned int from, const Page_Table * pt,
		    unsigned int n, CPU_Flags flags) {
    ...
    {
    public:
	DMA_Buffer(unsigned int s) : Chunk(s, CPU_Flags::DMA) {
	    Directory dir(current());
	    _log_addr = dir.attach(*this);
	    db<CPU_MMU>(TRC) << "CPU_MMU::DMA_Buffer() => "
			      << *this << "\n";
	}
	DMA_Buffer(unsigned int s, const Log_Addr & d)
	    : Chunk(s, CPU_Flags::DMA) {
	    Directory dir(current());
	    _log_addr = dir.attach(*this);
	    memcpy(_log_addr, d, s);
	    db<CPU_MMU>(TRC) << "CPU_MMU::DMA_Buffer(phy=" << *this
			      << " <= " << d <<"\n";
	}
    ...
    typedef Page_Table Page_Directory_Pointer;
    typedef PT_Entry PDP_Entry;
    typedef Page_Table Page_Map_Lvl4;
    typedef PT_Entry PML4_Entry;
...
public:
	...
	if(frames) {
		...
		db<CPU_MMU>(WRN) << "CPU_MMU::alloc() failed!\n";
	}
        db<CPU_MMU>(TRC) << "CPU_MMU::alloc(frames=" << frames << ") => "
			  << (void *)phy << "\n";
	return phy;
    }
	...
	frame = indexes(frame); 
        ...
        db<CPU_MMU>(TRC) << "CPU_MMU::free(frame=" << (void *)frame
			  << ",n=" << n << ")\n";
...
//        db<CPU_MMU>(TRC) << "CPU_MMU::flush_tlb()\n";

//	ASMV("movl %cr3,%eax");
//	ASMV("movl %eax,%cr3");
    }
    static void flush_tlb(Log_Addr addr) {
//        db<CPU_MMU>(TRC) << "CPU_MMU::flush_tlb(" << addr << ")\n";
