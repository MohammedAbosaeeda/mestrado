...
void Thread::common_constructor(Log_Addr entry, unsigned int stack_size) 
{
    ...
    _sched_link = _scheduler.insert(this, _wait_link.rank());
    if((_state != READY) && (_state != RUNNING))
	_scheduler.suspend(this->link());
...
void Thread::priority(const Priority & p)
{
    ...
    _wait_link.rank(int(p));
    _scheduler.update_rank(this->link(),int(p));

    if((_state != SUSPENDED) || (_state != WAITING))
    _scheduler.suspend(this->link());
    _scheduler.resume(this->link());
...
void Thread::pass()
{
    ...
    Thread * prev = running();
    Thread * next = _scheduler.choose(this->link());
...
void Thread::exit(int status)
{
    ...
    *static_cast<int *>(thr->_stack) = status;
	...
	_scheduler.resume(thr->_joining->link());
	thr->_joining = 0;
    }
...
void Thread::sleep(Queue * q)
{
    ...
    _scheduler.suspend(thr->link());
    thr->_state = WAITING;
    q->insert(&thr->_wait_link);
    thr->_waiting = q;
...
void Thread::wakeup(Queue * q) 
{
    ...
    if(!q->empty()) {
        Thread * t = q->remove().get()->object();
        t->_state = READY;
        t->_waiting = 0;
        _scheduler.resume(t->link());
    }
