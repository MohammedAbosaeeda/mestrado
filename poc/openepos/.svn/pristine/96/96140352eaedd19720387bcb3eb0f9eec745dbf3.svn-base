...
heap_id_t array_new(nvm_int_t length, u08_t type) {
  ...
  id = HeapVM::getInstance()->heap_alloc(false, 1 + length * array_typelen(type));

  // store type in first byte
  *(u08_t*)(HeapVM::getInstance()->heap_get_addr(id)) = type;

  return id;
}

nvm_int_t array_length(heap_id_t id) {
  // DEBUGF("arraylength %d = %d/%d\n", id, heap_get_len(id)-1, array_typelen(*(u08_t*)heap_get_addr(id)));

    return((HeapVM::getInstance()->heap_get_len(id)-1)/
           array_typelen(*(u08_t*) HeapVM::getInstance()->heap_get_addr(id)));
}
 
void array_bastore(heap_id_t id, nvm_int_t index, nvm_byte_t value) {
    nvm_byte_t * ptr = (nvm_byte_t *) HeapVM::getInstance()->heap_get_addr(id) + 1;
...
void array_iastore(heap_id_t id, nvm_int_t index, nvm_int_t value) {
    nvm_int_t * ptr = (nvm_int_t *)((u08_t*) HeapVM::getInstance()->heap_get_addr(id) + 1);
...
void array_fastore(heap_id_t id, nvm_int_t index, nvm_float_t value) {
    nvm_float_t * ptr = (nvm_float_t*)((u08_t*)HeapVM::getInstance()->heap_get_addr(id) + 1);
