...
static unsigned int M_PKT     = 0x03;      // missing packet
...
void send_packets(Packet * p, int number, int size) {
    ...
    while (p[number].num <= size) {
        nic.send(0, 0, &p[number], sizeof(p[number]));

        if (nic.receive(0, 0, &m_pkt, sizeof(Message)) && (m_pkt.type == M_PKT) && (m_pkt.content == version)) {
            send_packets(p, m_pkt.content, size);
            return;
        }
