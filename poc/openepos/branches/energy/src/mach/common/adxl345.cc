// OpenEPOS ADXL345 Mediator Implementation

#include <mach/common/adxl345.h>
#include <alarm.h>
...
void ADXL345::evaluateMetersPerSec() {
  ...
  for (short axis = 0; axis <= 2; axis++) {
    meterPerSecSec[axis] = (accelSample[axis] / accelSampleCount) * accelScaleFactor[axis] + runTimeAccelBias[axis];
        accelSample[axis] = 0;
        db<ADXL345>(TRC) << "meterPerSecSec[" << axis << "] = " << meterPerSecSec[axis] << endl;
  }
  accelSampleCount = 0;
}
...
void ADXL345::computeAccelBias(){
    db<ADXL345>(TRC) << "--- computeAccelBias ---" << endl;
    ...
    for (int samples = 0; samples < SAMPLECOUNT; samples++) {
     ...
     for (int axis = 0; axis < 3; axis++) {
       ...
       accelSample[axis] = 0;
       db<ADXL345>(TRC) << "accelSample/SAMPLECOUNT["<<axis<<"] = " << accelSample[axis]/SAMPLECOUNT << endl;
       db<ADXL345>(TRC) << "accelScaleFactor["<<axis<<"] = " << accelScaleFactor[axis] << endl;
       db<ADXL345>(TRC) << "meterPerSecSec["<<axis<<"] = " << meterPerSecSec[axis] << endl;
       db<ADXL345>(TRC) << "runtTimeAccelBias["<<axis<<"] = " << runTimeAccelBias[axis] << endl;
     }
     ...
     db<ADXL345> (TRC) << " --- runTimeAccelBias after calibration --- " << endl;
     for (char axis = 0; axis < 3; axis++)
         db<ADXL345>(TRC) << "runtTimeAccelBias["<<axis<<"] = " << runTimeAccelBias[axis] << endl;

     accelOneG = Math::abs(meterPerSecSec[2] + runTimeAccelBias[2]);
     db<ADXL345>(TRC) << "accelOneG = " << accelOneG << endl;

}



ADXL345::ADXL345() : accelOneG(-9.80665),
...
void ADXL345::write(unsigned int address, unsigned int value){
...
int ADXL345::read(char registerAddress){
     SPI::spiErr_t e;
    unsigned int data[] = {0};
...
int ADXL345::format(char add0, char add1){
    int value;
...
void ADXL345::initializeAccel(int range, int sample_rate){
