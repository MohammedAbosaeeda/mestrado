...
void reset_thread_counters()
{
    for(int i = 0; i < Traits<Machine>::MAX_CPUS; i++) {
        Thread::_lock_counter[i] = 0;
        for(int j = 0; j < Thread::MAX_COUNTING; j++) 
            Thread::_lock_tsc[i][j];
    }
}
...
void print_all_thread_stats()
{
  for(int i = 0; i < Traits<Machine>::MAX_CPUS; i++) {
        ...
        cout << "CPU [" << i << "] WCET = " << total_wcet << " AVG = " << total_mean 
        << " VAR = " << mean_stdev << "\n"; 
    }
}
...
void collect_thread_stats(int test)
{
    ...
    for(int i = 0; i < Traits<Machine>::MAX_CPUS; i++) {
        var = 0;
        stats.mean[i][test] = mean((TSC::Time_Stamp *) &Thread::_lock_tsc[i], Thread::_lock_counter[i], &stats.wcet[i][test]);
        var = variance((long long unsigned int *) &Thread::_lock_tsc[i],  Thread::_lock_counter[i], stats.mean[i][test]);
