...
PROXY_BEGIN(DTMF_Detector)
    ...
    char do_dtmf_detection(){
        return Base::call_r<DTMF_Detector::OP_DO_DTMF_DETECTION,char>();
    }
PROXY_END

AGENT_BEGIN(DTMF_Detector)
    D_CALL_R_1(add_sample, OP_ADD_SAMPLE, bool, sample_t)
    D_CALL_R_0(do_dtmf_detection, OP_DO_DTMF_DETECTION, char)
AGENT_END

};

DECLARE_COMPONENT(DTMF_Detector);
