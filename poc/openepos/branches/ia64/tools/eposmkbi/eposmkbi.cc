...
int main(int argc, char **argv)
{
    ...
    // Add SETUP LVL1 (SETUP)
    unsigned int SETUP1_MAX_LENGTH = 0x1000;
    si.bm.setup_offset = image_size - boot_size;
    printf("si.bm.setup_offset = %d\n\n", si.bm.setup_offset);
    sprintf(file, "%s/img/%s_setup", argv[1], CONFIG.mach);
    printf("    Adding setup \"%s\":", file);
    image_size += put_file(fd_img, file);
    if(image_size > SETUP1_MAX_LENGTH) {
        printf(" failed!\n");
        fprintf(stderr, "Setup lvl 1 \"%s\" is too large! (%d bytes)\n", 
            file, image_size);
        return 1;
    } else {
        while((image_size % SETUP1_MAX_LENGTH != 0))
            image_size += pad(fd_img, 1);
    }

    // Add SETUP LVL2 (SETUP64)
    si.bm.setup64_offset = image_size - boot_size;
    printf("si.bm.setup64_offset = %d\n", si.bm.setup64_offset);
    sprintf(file, "%s/img/%s_setup64", argv[1], CONFIG.mach);
    printf("    Adding setup64 \"%s\":", file);
    image_size += put_file(fd_img, file);
    ...
    if(need_si) {
        ...
        // System info data typed doesnt change, so we dont need to change the
        // type here.
        case 32: if(!add_boot_map<int>(fd_img, &si)) return 1; break;
        case 64: if(!add_boot_map<int>(fd_img, &si)) return 1; break;
        default: return 1;
        }
    }
...
template<typename T> bool add_boot_map(int fd, System_Info * si)
{
    ...
    if(!put_number(fd, static_cast<T>(si->bm.setup_offset)))
        return false;
    if(!put_number(fd, static_cast<T>(si->bm.setup64_offset)))
        return false;
