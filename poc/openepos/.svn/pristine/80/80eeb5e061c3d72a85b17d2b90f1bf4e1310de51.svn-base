...
public:
    ...
    int send(const Address & dst, const Protocol & prot,
		...
		acquire_lock(_send_lock);
		acquire_lock(_state_machine_lock);
		acquire_lock(_tx_pending);

		/*
        if (Traits<CMAC<T> >::time_triggered) {
		//	while(_tx_pending || _busy) cmac_yield();
            if (_tx_pending)
			{
                kout << "CMAC::send - _tx_pending\n";
                return -1;
            }
            if (size > mtu())
			{
                kout << "CMAC::send - size > mtu()\n";
                return -1;
            }
            if (_busy)
			{
                kout << "CMAC::send - _busy\n";
                return -1;
            }
        ...
        if (Traits<CMAC<T> >::time_triggered) {
			...
			release_lock(_state_machine_lock);
			acquire_lock(_tx_pending); // state machine will release this lock
			release_lock(_tx_pending);
//            while (_busy) cmac_yield();
    ...
    int receive(Address * src, Protocol * prot,
		...
		if (size > mtu()) {
			db<CMAC<T> >(WRN) << "CMAC::receive - buffer size > MTU\n";
			return -1;
		}
		...
		acquire_lock(_receive_lock);
		acquire_lock(_state_machine_lock);
		acquire_lock(_rx_pending);
        ...
        if (!_buffer_empty) {
            ...
            *prot = _rx_protocol;
            ...
            if (result == UNPACK_OK) {
				...
				release_lock(_rx_pending);
				release_lock(_state_machine_lock);
				release_lock(_receive_lock);
        ...
        if (Traits<CMAC<T> >::time_triggered) {
            ...
            _busy = true;
			release_lock(_state_machine_lock);
			acquire_lock(_rx_pending); //state machine will release this lock
			release_lock(_rx_pending);
            //while (_busy) cmac_yield();
        ...
        *prot = _rx_protocol;
        ...
        if (result == RX_OK) {
			...
			unsigned int ret = _rx_data_size;
			release_lock(_receive_lock);
            if (!Traits<CMAC<T> >::time_triggered)
                notify((int) *prot);
...
protected:
	...
	static volatile bool _state_machine_lock;
	static volatile bool _send_lock;
	static volatile bool _receive_lock;
    static void acquire_lock(volatile bool & lock) { while(CPU::tsl(lock)) cmac_yield();}
    static void release_lock(volatile bool & lock) { lock = false; }
            ...
            switch (state) {
                ...
                case TX_PREAMBLE:
                    if (Traits<CMAC<T> >::time_triggered) {
                        ...
                        preamble->time = (Traits<CMAC<T> >::SLEEPING_PERIOD + Traits<CMAC<T> >::SLEEPING_PERIOD/2) * 1000;// + (Traits<CMAC<T> >::SLEEPING_PERIOD >> 1) + 10;
                        preamble->rank = 0;

                        CMAC<T>::_frame_buffer_size = sizeof(preamble_t);

//						unsigned long begin = CMAC<T>::alarm_ticks_ms;
						unsigned long end = preamble->time;
						unsigned volatile long last = 0;
						c.reset();
						c.start();
						CMAC<T>::_radio->gpio_on();
						do
						{
							//while(CMAC<T>::alarm_ticks_ms <= last);
                            Generic_Tx<T>::execute(result);
							last = c.read();
							preamble->time = end - last;
						} while(last < end);
						CMAC<T>::_radio->gpio_off();
//						kout << "Preamble sent!\n";
                ...
                case RX_PREAMBLE:
                    if (Traits<CMAC<T> >::time_triggered) {
						...
						if (CMAC<T>::_buffer[CMAC<T>::_buffer_tail].size == sizeof(preamble_t)) {
							...
							if (p->time <= (Traits<CMAC<T> >::SLEEPING_PERIOD + Traits<CMAC<T> >::SLEEPING_PERIOD/2)*1000/* + (Traits<CMAC<T> >::SLEEPING_PERIOD >> 1) + 10*/) {
								CMAC<T>::_radio->gpio_on();
								CMAC<T>::_radio->off();
								// This offset was experimentaly calculated with an oscilloscope for 2 eposmotes.
								// The oscilloscope indicates that, with this offset, the two motes will 
								// exit the loop with at most an 125us difference
								wait_us((int)p->time-(int)667);
//								wait_us(500);
...
private:
    static void wait_ms(unsigned int ms) {
		Chronometer c;		
		c.start();
		while(c.read()/1000 < ms);
    }
    static void wait_us(int us) {
		Chronometer c;
		c.start();
		while(c.read() > 10); // Chronometer sometimes returns a large value if read too fast
		while(c.read() < us);
    }
};
...
public:
    ...
    static CMAC_STATE_TRANSITION execute(CMAC_STATE_TRANSITION input) {
        if (Traits<CMAC<T> >::BACKOFF > 0)
            execute(input, Traits<CMAC<T> >::BACKOFF + (Pseudo_Random::random() % Traits<CMAC<T> >::BACKOFF));
...
//        if (CMAC<T>::_frame_buffer_size == 0) {
        if (CMAC<T>::_buffer[CMAC<T>::_buffer_head].size == 0) {
            ...
            //kout << "IEEE802154_Unpack - UNPACK_FAILED - Frame size == 0\n";
        ...
        //kout << "IEEE802154_Unpack - Decoding frame\n";
...
//        unsigned short crc = CRC::crc16(reinterpret_cast<char*>(CMAC<T>::_frame_buffer), CMAC<T>::_frame_buffer_size - 2);
        ...
        if (crc_frame != crc) {
            ...
            //kout << "IEEE802154_Unpack - CRC error: " << crc << "\n";
			return unpack_failed();
            ...
            //kout << "IEEE802154_Unpack - UNPACK_FAILED - Incorrect frame type\n";
			input = unpack_failed();
            ...
            //kout << "IEEE802154_Unpack - UNPACK_FAILED - Wrong address\n";
			input = unpack_failed();
            ...
            //kout << "IEEE802154_Unpack - UNPACK_OK\n";
        }
