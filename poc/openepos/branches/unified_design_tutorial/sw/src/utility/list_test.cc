...
using Unified::Simple_List;
using Unified::Simple_Grouping_List;
using Unified::List;
using Unified::Ordered_List;
using Unified::Relative_List;
...
void test_simple_list ()
{
    ...
    cout << "Removing the element whose value is " << o[N/2] << " => " 
	 << *l.remove(&o[N/2]).get()->object() << "\n";
    cout << "Removing the list's head => " << *l.remove_head().get()->object()
	 << "\n";
    cout << "Removing the element whose value is " << o[N/4] << " => " 
	 << *l.remove(&o[N/4]).get()->object() << "\n";
    cout << "Removing the list's tail => " << *l.remove_tail().get()->object()
	 << "\n";
    cout << "Trying to remove an element that is not on the list => " 
	 << l.remove(&o[N+1]).get() << "\n";
    cout << "Removing all remaining elements => ";
    while(l.size() > 0) {
	cout << *l.remove().get()->object();
	if(l.size() > 0)
	    cout << ", ";
    }
    cout << "\n";
...
void test_simple_grouping_list() 
{
    ...
    d1 = l.search_decrementing(1).get();
    if(d1) {
    ...
    d1 = l.search_decrementing(6).get();
    if(d1) {
	cout << *(d1->object() + d1->size());
	if(!d1->size()) {
	    cout << "[rm]"; // removed
    ...
    d1 = l.search_decrementing(N * 2).get();
    if(d1) {
	cout << *(d1->object() + d1->size());
	if(!d1->size()) {
	    cout << "[rm]"; // removed
    ...
    d1 = l.search_decrementing((N * 2) - 7).get();
    if(d1) {
	cout << *(d1->object() + d1->size());
	if(!d1->size()) {
	    cout << "[r]"; // removed
...
void test_scheduling_list ()
{
    ...
    cout << "\n";
    cout << "Scheduling the list => " << *l.choose().get()->object() << "\n";
    cout << "They are: ";
    for(Scheduling_List<int>::Iterator i = l.begin(); i != l.end(); i++) {
	cout << *i->object();
	if(Scheduling_List<int>::Iterator(i->next()) != l.end())
	...
	*l.choose_another().get()->object() << "\n";
    ...
    cout << "\n";
    cout << "Removing the list's head => " << *l.remove(l.choose().get())->object()
	 << "\n";
    cout << "Removing the element whose value is " << o[N/4] << " => " 
	 << *l.remove(e[N/4])->object() << "\n";
    cout << "Removing all remaining elements => ";
    while(l.size() > 0) {
	cout << *l.remove(l.choose().get())->object();
	if(l.size() > 0)
	    cout << ", ";
    }
    cout << "\n";
