...
SC_MODULE(RSP) {
    ...
    typedef RSP_ETH_Node<ROUTER_X,ROUTER_Y, Implementation::Resource_Table<Implementation::RSP_ETH,0>::HW_LOCAL,
                     NET_DATA_WIDTH,NET_SIZE_X_LOG2,NET_SIZE_Y_LOG2,
                     RMI_MSG_SIZE,
                     IID_SIZE, Implementation::Traits<Implementation::RSP_ETH>::n_ids> rsp_eth_node_t;
	...
	sc_signal<unsigned int> iid_rsp_eth[Implementation::Traits<Implementation::RSP_ETH>::n_ids];
		 ...
		 , node_rsp_dtmf("node_rsp_dtmf")*/
	{
        ...
        noc_connect<Implementation::RSP_ETH, rsp_eth_node_t>(node_rsp_eth, iid_rsp_eth);
