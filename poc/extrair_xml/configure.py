    ...
    if len(configs) > 0:
      ...
      with open('%s/tap_%s.sh' % (folder, app_name), 'a') as f:
        ...
        for key in options_map:
          count += 1
          f.write('%sfor %s in "${%s_options[@]}" do \n' % ('\t' * count, key.lower(),key.lower()))
          if options_map[key][0]:
            f.write('\t%ssed -i "/template <> struct Traits<%s>: public Traits<void>/,/template <> struct/ {s/static const unsigned int %s.*/static const unsigned int %s = $%s;/g}" $trait \n' % ('\t' * count, options_map[key][0], key, key, key.lower()))
          else:
        ...
        f.write("echo `date -r $DIFF +'%H:%M:%S'` >> ${email_body}\n")
      f.closed
