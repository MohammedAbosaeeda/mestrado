...
public:
	...
	~Chunk() {
	    //if not IO, then we won't free mapped memory (it is on the IO AS)
	    if(!(_flags & IA32_Flags::IO)) {
	        //if frames are contiguous, then free it all together
            if(_flags & IA32_Flags::CT) {
                free((*_pt)[_from], _to - _from);
            }
            //if frames aren't contiguous, then free them individually
            else {
	    ...
	    //free frames holding the page tables
	    free(_pt, _pts);
	}
