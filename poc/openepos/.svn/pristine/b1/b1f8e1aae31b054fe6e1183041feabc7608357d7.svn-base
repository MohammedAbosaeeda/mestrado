                 ...
                 IP::Address dst, void *data, unsigned int size)
{
    ...
    if (!(hdr.validate_checksum(src,dst,size - hdr.size()))) {
        cccout << "TCP checksum failed for incoming packet!\n";
        return;
    }

    int len = size - hdr.size();
    if (len < 0) {
        cccout << "Misformed TCP segment received\n";
        return;
    }
                         ...
                         TCP::Address dst, void *data, unsigned int size)
{
    ...
    if ((_remote == src) || (_remote.port() == 0))
    {
        ...
        cccout << "TCP Segment does not belong to us\n";          
    }
}
...
void TCP::Socket::connect() {
	cccout << "socket::connect\n";
    if (state() != CLOSED && state() != SYN_SENT) {
        cccout << "TCP::Socket::connect() could not be called\n";
        return;
    }
...
void TCP::Socket::listen()
{
    if (state() != CLOSED && state() != LISTEN) {
        cccout << "TCP::Socket::listen() called with state: " << state() << endl;
        return;
    }
...
{
    cccout << __PRETTY_FUNCTION__ << endl;
...
void TCP::Socket::__ESTABLISHED(const Header& r ,const char* data,u16 len)
{
    ...
    if (r._rst) {
        ...
        cccout << "TCP::out of order segment received\n";
    }
}
...
TCP::Channel::~Channel() {
    if (state() != CLOSED) {
        cccout << "Destroying non-closed channel!\n";
...
bool TCP::Channel::connect(const TCP::Address& to)
{
	...
	cccout << "TCP::Channel::connect()\n";
    if (state() != CLOSED) {
        cccout << "TCP::Channel::connect() called for open connection!\n";
        return false;
    }
...
int TCP::Channel::receive(char * dst,unsigned int size)
{
    ...
    if (_receiving) {
        cccout << "TCP::Channel::receive already called!\n";
        return -ERR_ILEGAL;
    }
...
{
    if (state() != CLOSED) {
        cccout << "Cannot use TCP::Channel::bind() on open connection\n";
        return;
    }
...
{
    if (state() != CLOSED) {
        cccout << "TCP::Channel::listen() called on non-closed channel\n";
        return false;
    }
...
void TCP::Channel::received(const char* data,u16 size)
{
    ...
    if (!_rx_buffer_ptr || (remaining == 0)) {
        cccout << "Channel::received droping data, no buffer space\n";
        return;
    }
    if (remaining < static_cast<int>(size)) {
        cccout << "Channel::received data truncated\n";
