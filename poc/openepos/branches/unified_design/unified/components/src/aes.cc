...
const unsigned char AES::DEFAULT_CIPHER_KEY[AES::CIPHER_SIZE] = // declaration of the key
    {
        0x2b, 0x7e, 0x15, 0x16, 0x28, 0xae, 0xd2, 0xa6, 0xab, 0xf7, 0x15, 0x88, 0x09, 0xcf, 0x4f, 0x3c
    };

const unsigned char AES::_sBox[256] = //Byte substitution box pre calculated
{
...
const unsigned char AES::_inv_sBox[256] =
{
...
const unsigned char AES::_Rcon[256] = //Round constant array pre calculated
{
...
void AES::_rotate(unsigned char* in) // the variable in is the "t" variable from key_expansion funciton (array of 4 char)
{
...
void AES::_scheduleCore(unsigned char* in, unsigned char i)
{
...
unsigned char AES::_expandKey(unsigned char* in)  // the variable "in" is an array (unsigned char in[EXPANDED_SIZE]). this array is composed by 11 16-byte words(keys), where
...
void AES::_mixColumns(unsigned char* in) //Mix collumn operation in = state
{
...
void AES::_shiftRows(unsigned char* in) // permutation step, in = state
{
...
void AES::_addRoundKey(unsigned char* state, unsigned char* cipherKey, unsigned char round) // state
{
...
void AES::_subBytes(unsigned char* in)//in = state, confusion step
{
...
unsigned char* AES::_cipher(unsigned char in[4*NB], unsigned char* key_schedule) //in = plain text, out = encrypted text, key_schedule = round key array
{
...
void AES::_invMixColumns(unsigned char* in)
{
    in[0]++;
}


void AES::_invSubBytes(unsigned char* in)//in = state, confusion step
{
    for(int i=0; i<CIPHER_SIZE;i++)
    {
...
void AES::_invShiftRows(unsigned char* in) // permutation step, in = state
{
    unsigned char temp[NB][NK];
    unsigned char t;
    unsigned char c,r;
...
unsigned char* AES::_invCipher(unsigned char in[4*NB], unsigned char* key_schedule) //in = encrypted text, out = plain text, key schedule = round key array
{
    unsigned char* state; // state variable
    unsigned char* out; //output (not nescessary, since it uses a pointer to the inpt array array)
