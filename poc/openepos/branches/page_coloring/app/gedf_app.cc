...
void reset_ipi_counters(void);
void collect_ipi_stats(int);
void collect_alarm_stats(int);
void print_all_ipi_stats(void);
...
typedef struct {
    TSC::Time_Stamp mean_alarm[TEST_REPETITIONS];
    TSC::Time_Stamp var_alarm[TEST_REPETITIONS];
    TSC::Time_Stamp wcet_alarm[TEST_REPETITIONS];
    int counter;
} alarm_stats;

alarm_stats stats[Traits<Machine>::MAX_CPUS];
...
int run(int test)
{
    ...
    for(int i = 0; i <  THREADS; i++) {
        threads[i] = new Periodic_Thread(&job, 
                                         ...
                                         i, //ID
                                         (RTC::Microsecond) threads_parameters[i][0], // period
                                         Thread::Criterion((RTC::Microsecond) threads_parameters[i][0]), //criterion
...
/*void print_alarm(void)
{
    ...
    for(int i = 0; i < Traits<Machine>::MAX_CPUS; i++) {
      ...
      cout << "[" << i << "]" << " ticks = " << SMP_Alarm::_tick_couting[i] << " mean=" << m;
      cout << " WCET=" << wcet;
...
/*void reset_ipi_counters()
{
    /*Thread::_ipi_sender = 0;
    Thread::_ipi_receiver = 0;
    for(int i = 0; i < Thread::MAX_COUNTING; i++) {
        Thread::_ipi_sender_tsc[i] = 0;
        Thread::_ipi_receiver_tsc[i] = 0;
    }
    Thread::_ipi_counter = 0;
    ...
    for(int i = 0; i < Traits<Machine>::MAX_CPUS; i++) {
        Thread::_ipi_per_cpu[i] = 0;
        Thread::_cpu_ipi_receiver[i] = -1;
        Thread::_cpu_ipi_sender[i] = -1;
    }
}*/
...
void collect_alarm_stats(int test)
{
    ...
    for(int i = 0; i < Traits<Machine>::MAX_CPUS; i++) {
      if(SMP_Alarm::_tick_couting[i]) {
        ...
        //cout << " STD=" << s << endl;
        stats[i].counter++;
        stats[i].mean_alarm[stats[i].counter] = m;
        stats[i].wcet_alarm[stats[i].counter] = wcet;
        stats[i].var_alarm[stats[i].counter] = var;
      } else {
          cout << "CPU " << i << " did not have any alarm\n";
      }
    } 
...
{ 
    ...
    for(int i = 0; i < Traits<Machine>::MAX_CPUS; i++) {
        TSC::Time_Stamp total_wcet = find_highest(stats[i].wcet_alarm, stats[i].counter);
        unsigned long long total_mean = mean_wo_mul(stats[i].mean_alarm, stats[i].counter, &tmp);
        unsigned long long total_var =  mean_wo_mul(stats[i].var_alarm, stats[i].counter, &tmp);
        cout << "CPU[" << i << "] WCET = " << total_wcet << " AVG = " << total_mean << " VAR = " << total_var << "\n"; 
    }
}

/*typedef struct {
    TSC::Time_Stamp mean_ipi[TEST_REPETITIONS];
    TSC::Time_Stamp var_ipi[TEST_REPETITIONS];
    TSC::Time_Stamp wcet_ipi[TEST_REPETITIONS];
} ipi_stats;

ipi_stats stats;

TSC::Time_Stamp ipi_lat[Thread::MAX_COUNTING];*/

//void collect_ipi_stats(int test)
//{
    /*cout << "Total IPI sent = " << Thread::_ipi_sender <<  " received = " << Thread::_ipi_receiver << "\n";
    
    for(int i = 0 ; i < Thread::_ipi_receiver; i++) {
        ipi_lat[i] = Thread::_ipi_receiver_tsc[i] - Thread::_ipi_sender_tsc[i];
    }
    
    if(Thread::_ipi_receiver != 0) {
        stats.mean_ipi[test] = mean(ipi_lat, Thread::_ipi_receiver, &stats.wcet_ipi[test]);
        stats.var_ipi[test] = variance(ipi_lat, Thread::_ipi_receiver, stats.mean_ipi[test]);
    } else {
        stats.mean_ipi[test] = 0;
        stats.var_ipi[test] = 0;
        stats.wcet_ipi[test] = 0;
    }*/
    
    /*cout << "Total IPI sent = " << Thread::_ipi_counter << "\n";
    
    if(Thread::_ipi_counter != 0) {
        stats.mean_ipi[test] = mean(Thread::_ipi_tsc, Thread::_ipi_counter, &stats.wcet_ipi[test]);
        stats.var_ipi[test] = variance((TSC::Time_Stamp *)Thread::_ipi_tsc, Thread::_ipi_counter, stats.mean_ipi[test]);
    } else {
        stats.mean_ipi[test] = 0;
        stats.var_ipi[test] = 0;
        stats.wcet_ipi[test] = 0;
    }
    cout << "WC = " << stats.wcet_ipi[test] << " AVG = " 
        << stats.mean_ipi[test] << " VAR = " << stats.var_ipi[test] << "\n";
}

void print_all_ipi_stats()
{ 
    unsigned long long tmp;
    TSC::Time_Stamp total_wcet = find_highest(stats.wcet_ipi, TEST_REPETITIONS);
    unsigned long long total_mean = mean_wo_mul(stats.mean_ipi, TEST_REPETITIONS, &tmp);
    unsigned long long total_var =  mean_wo_mul(stats.var_ipi, TEST_REPETITIONS, &tmp);
  
    cout << "WCET = " << total_wcet << " AVG = " << total_mean << " VAR = " << total_var << "\n";  
}*/
