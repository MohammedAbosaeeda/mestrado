    ...
    if len(configs) > 0:
      ...
      if debug:
        ...
        if len(debug_filepath_tag) > 0:
          debug_filepath = debug_filepath_tag[0].childNodes[0].data
      ...
      for trait_tag in traits:
        ...
        options.append('%s_options=(%s);' % (trait_id.lower(), ' '.join(values)))
...
#        f.write('START=`date +%H:%M`\n')
        ...
        if debug:
          f.write('\t\t%sscreen -S qemu -X quit\n'% ('\t' * count))
          f.write('\t\t%sscreen -S gdb -X quit\n'% ('\t' * count))
          ...
          if debug_filepath:            
            ...
            f.write('\t\t%secho "Para conectar ao QEMU utilizar target remote:1234"\n' % ('\t' * count))
            f.write('\t\t%sgdb \n' % ('\t' * count))
...
if __name__ == "__main__":
  ...
  parser = argparse.ArgumentParser(description='Process XML file and outputs scripts.')
  parser.add_argument('--filename', required=True, help='The XML configuration filename.')
  parser.add_argument('--epospath', required=True, help='The absolute path for EPOS.')
  parser.add_argument('--execute', help='Execute TAP after configuration.')
