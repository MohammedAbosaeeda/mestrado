// OpenEPOS EMote2ARM_I2C Mediator Implementation
...
EMote2ARM_I2C::EMote2ARM_I2C()
 : _scl(I2C_SCL_GPIO, GPIO_Pin::FUNC_ALT1),
   _sda(I2C_SDA_GPIO, GPIO_Pin::FUNC_ALT1),
   _sync_timeout(0)
{
    db<EMote2ARM_I2C>(TRC) << "EMote2ARM_I2C::EMote2ARM_I2C()\n";
...
void EMote2ARM_I2C::enable()
{
    db<EMote2ARM_I2C>(TRC) << "void EMote2ARM_I2C::enable()\n";
...
void EMote2ARM_I2C::disable()
{
    db<EMote2ARM_I2C>(TRC) << "void EMote2ARM_I2C::disable()\n";

    CPU::out8(Machine::IO::I2C_CR, 0x00); // disable everything
    CPU::out8(Machine::IO::I2C_CKER, 0); // disable clock signal to i2c module
}

bool EMote2ARM_I2C::busy()
{
    db<EMote2ARM_I2C>(TRC) << "bool EMote2ARM_I2C::busy()\n";

    return ((CPU::in8(Machine::IO::I2C_SR) & (1 << I2C_SR_MBB)) > 0);
}

void EMote2ARM_I2C::set_timeout(TSC::Time_Stamp tm) {
    _sync_timeout = tm;
}

bool EMote2ARM_I2C::sync()
{
    if(_sync_timeout == 0)
        ...
        while(!(CPU::in8(Machine::IO::I2C_SR) & (1 << I2C_SR_MCF)))
        {
            ...
            {
                db<EMote2ARM_I2C>(ERR) << "I2C error: Sync timeout! Will reset hardware.\n";
                disable();
...
bool EMote2ARM_I2C::put_byte(unsigned char b)
{
    db<EMote2ARM_I2C>(TRC) << "bool EMote2ARM_I2C::put_byte(" << b << ")\n";
    ...
    if (CPU::in8(Machine::IO::I2C_SR) & (1 << I2C_SR_RXAK)) {
        db<EMote2ARM_I2C>(ERR) << "I2C error: RXACK missed when putting "<< b << ".\n";
        success = false;
    }
    ...
    if (CPU::in8(Machine::IO::I2C_SR) & (1 << I2C_SR_MAL)) {
        ...
        db<EMote2ARM_I2C>(ERR) << "I2C error: Arbitration lost.\n";
        success = false;
    }
...
bool EMote2ARM_I2C::get_byte(unsigned char * b, bool set_nack)
{
    db<EMote2ARM_I2C>(TRC) << "bool EMote2ARM_I2C::get_byte(...)\n";
...
bool EMote2ARM_I2C::write_register(
        unsigned char dst,
        unsigned char reg,
        int len,
        unsigned char * buff)
{
    db<EMote2ARM_I2C>(TRC) << "void EMote2ARM_I2C::write_register("
            << dst << ", "
            << reg << ", "
            << len << ", "
            << (void*) buff << ")\n";
...
bool EMote2ARM_I2C::read_register(
        unsigned char dst,
        unsigned char reg,
        int len,
        unsigned char * buff)
{
    db<EMote2ARM_I2C>(TRC) << "void EMote2ARM_I2C::read_register("
            << dst << ", "
            << reg << ", "
            << len << ", "
            << (void*) buff << ")\n";
