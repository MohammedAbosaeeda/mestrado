      ...
      _times(times), _link(this, _ticks)
{
    db<Alarm>(TRC) << "Alarm(t=" << time 
		   ...
		   << ",h=" << (void *)handler
		   << ",x=" << times << ") => " << this << "\n";
	db<Alarm>(TRC) << "Alarm(period()=" << period() << "\n";
    if(_ticks) {
		CPU::int_disable();
...
Alarm::Alarm(const Microsecond & time, Handler * handler, int times, bool int_enable)
    : _ticks((time + period() / 2) / period()), _handler(handler),
      _times(times), _link(this, _ticks)
{
    db<Alarm>(TRC) << "Alarm(t=" << time 
...
{
    db<Alarm>(TRC) << "Alarm::master(t=" << time << ",h=" << (void *)handler << ")\n";
    _master = handler;
    _master_ticks = (time + period() / 2) / period();
	//CPU::show_stuff((char *) 0x10040000);
}
...
void Alarm::int_handler(unsigned int)
{
	...
	//db<Alarm>(TRC) << "alarm.cc --> _elapsed " << _elapsed << "\n";
    _elapsed++;
	
	//db<Alarm>(TRC) << "alarm.cc --> _elapsed " << _elapsed << "\n";
	//db<Alarm>(TRC) << "alarm.cc --> _master_ticks " << _master_ticks << "\n";
	//db<Alarm>(TRC) << "alarm.cc --> _master_ticks " << (void *) &_master_ticks << "\n";
	...
	//db<Alarm>(TRC) << "alarm.cc --> 1 " << "\n";
	if(_master_ticks && ((_elapsed % _master_ticks) == 0)) 
		_master();
	//db<Alarm>(TRC) << "alarm.cc --> 2 " << "\n";

	if(current)
		(*current)();
	//db<Alarm>(TRC) << "alarm.cc --> 3 " << "\n";
