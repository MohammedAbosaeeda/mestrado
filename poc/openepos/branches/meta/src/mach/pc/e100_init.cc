...
int E100::init(unsigned int unit, System_Info * si)
{
    ...
    PCI::header(loc, &hdr);
    if(!hdr) {
	db<E100>(WRN) << "E100::init: PCI header failed!\n";
	return 1;
    }
    db<E100>(INF) << "E100::init: PCI header=" << hdr << "}\n";
    if(!(hdr.command & PC_PCI::COMMAND_MEMORY))
	db<E100>(WRN) << "E100::init: I/O memory unaccessible!\n";
    if(!(hdr.command & PC_PCI::COMMAND_MASTER))
	db<E100>(WRN) << "E100::init: not master capable!\n";

    // Get I/O base port
    Log_Addr io_mem = hdr.region[PCI_REG_MEM].log_addr;
    db<E100>(INF) << "E100::init: I/O memory at " 
    ...
    db<E100>(INF) << "E100::init: PCI interrut pin "
