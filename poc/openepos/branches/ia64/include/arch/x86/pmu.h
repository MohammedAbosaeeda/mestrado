// EPOS CPU PMU Mediator Declarations
...
class CPU_PMU: public PMU_Common
{
public:  
...
public:
	...
	*eax = op;
    ...
    static void config(Reg32 event_reg, Reg32 flags) {
        db<CPU_PMU>(TRC) << "CPU_PMU::config()\n";
        wrmsr(event_reg, flags);
    }
    
    static Reg64 rdmsr(Reg32 reg) {
        db<CPU_PMU>(TRC) << "CPU_PMU::rdmsr()\n";
        Reg64 v;
    ...
    static void enable(Reg32 reg) {
        db<CPU_PMU>(TRC) << "CPU_PMU::enable()\n";
        wrmsr(reg, (rdmsr(reg) | ENABLE));
    }
    
    static void disable(Reg32 reg) {
        db<CPU_PMU>(TRC) << "CPU_PMU::disable()\n";
        wrmsr(reg, (rdmsr(reg) & ~ENABLE));
    }
...
class Intel_PMU_Version2 : public CPU_PMU
{
public:
    //Flags
    enum {
    ...
    static int config(Reg32 reg, Reg32 flags) {
        db<CPU_PMU>(TRC) << "Intel_Core_Duo_PMU::config()\n";
        if((reg - EVTSEL0) > num_counters()) {
            db<CPU_PMU>(WRN) << "Intel_Core_Duo_PMU::config() the number of available counters is " << num_counters() << "\n";
            return 0; //error
        }
    ...
    static bool overflow(Reg32 reg) {
        db<CPU_PMU>(TRC) << "CPU_PMU::overflow()\n";
...
public:
    ...
    //Layout of CPU_PEBS_ENABLE MSR figure 18-28
    enum {
        PS_EN = (0x01LLU << 63),
