...
int main()
{
	Poly1305 * poly = new Poly1305(new AES_Controller());
	bool ret;
	Alarm::delay(3000000);
	cout<<"Delay done\n";
	int i;
...
bool tst0(Poly1305* poly)
{
	...
	for(i=0;i<16;i++)
		if(k[i] != poly->_k.data[i]) 
			cout<<"Nop000! " << i << " " << (int)k[i] << " " << (int)poly->_k.data[i] << '\n';
	...
	for (loop = 0;loop < 10;++loop) {
		...
		for (;;) {
			...
			*/
			...
			x = Pseudo_Random::random() & 15;
			aux = out[x];
			do
				y = (Pseudo_Random::random() % 256);
			while(y == out[x]);
