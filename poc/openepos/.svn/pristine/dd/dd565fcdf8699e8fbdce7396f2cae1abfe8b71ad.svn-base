...
System::Chronometer chrono; // Time measurement
...
int main()
{
    ...
    cout << "bye!\n";
    cout << "Machine at " << System::IA32_TSC::frequency() <<"\n";
    while (true);
		...
		unsigned int dataSetReferencePicture)
{
    ...
    for (im = 0; im < timesMatch; ++ im) {
    	...
    	chrono.start(); // Time measurement
		pmc = pictureMotionEstimator->match(currentPicture, referencePicture);
		chrono.stop(); // Time measurement

		cout << "\nmatch took: " << chrono.read() << " micro seconds\n";
        cout << "Will now check ME results\n";
