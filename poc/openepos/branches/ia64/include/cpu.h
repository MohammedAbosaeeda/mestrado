...
public:
    ...
    public:
        ...
        Log_Addr(unsigned long long a) : _addr(a) {}
        template <typename T>
        Log_Addr(T * a) : _addr(reinterpret_cast<unsigned long long>(a)) {}

        operator unsigned long long() const { return _addr; }

        template <typename T>
        operator T *() const { return reinterpret_cast<T *>(_addr); }

        template <typename T>
        bool operator==(T a) const { return (_addr == (unsigned long long)a); }
        template <typename T>
        bool operator< (T a) const { return (_addr < (unsigned long long)a); }
        template <typename T>
        bool operator> (T a) const { return (_addr > (unsigned long long)a); }
        ...
        template <typename T>
        Log_Addr operator-(T a) const { return _addr - (unsigned long long)a; }
        template <typename T>
        Log_Addr operator+(T a) const { return _addr + (unsigned long long)a; }
        template <typename T>
