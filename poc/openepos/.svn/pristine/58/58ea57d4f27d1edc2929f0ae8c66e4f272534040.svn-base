...
public class IMLBitAnd extends IMBitAnd {


	/* copy constuctor */
	protected IMLBitAnd() { }

	public IMLBitAnd(IMMethod method, int bc, int bcpos) {
	...
	final public IMNode ssa_optimize() throws CompileException {
		...
		if (rOpr.isConstant()) {
			...
			if (value==0L) {
				return method.createIMLConstant(0L, bcPosition);
			}
		}
