...
public class CoderTricore extends Coder {

	private final boolean tricore_PMI_workaround = true;

	public CoderTricore(BuilderOptions opts, MethodTableFactory table,
	...
	protected void writeMakefile(PrintStream out) throws IOException {
		out.println("\n\n# default is the target that the shipped ProOSEK Makefiles build");
		out.println("# we will use the all target to append commands to the default build process");
		out.println("all: kesolib default");

		out.println("\t@ln -fs TriboardTC1796gnu/keso_main.elf");
		out.println("\t@cd TriboardTC1796gnu && cp keso_main.elf keso_main_g.elf");
		out.println("\t@ln -fs TriboardTC1796gnu/keso_main_g.elf");
		out.print("\t@cd .. && ln -fs ");
		out.print(opts.getSystemName());
		out.print("/keso_main_g.elf && ln -fs ");
		out.print(opts.getSystemName());
		out.println("/TriboardTC1796gnu/os.orti");
		out.println("\t@$(STRIP) keso_main.elf");
		out.println("\t@tricore-objcopy -R .version_info keso_main.elf\n");
		out.println("\t@echo $(KESORC) : $(JINOFLAGS)");
		out.println("\t@$(SIZE) keso_main.elf");
		out.println();

		out.println("clean: ");
		out.println("\t@rm -rf keso_main.elf keso_main_g.elf TriboardTC1796gnu\n");

		out.println("\n\n# include template with shipped ProOSEK build process");
		out.println("include $(KESOSRCPATH)/templates/Makefile.tricore-osek");
	...
	protected void writeRuntimeData(PrintStream out, Vector tasks) throws CompileException {
		...
		if (!opts.hasOption("no_inline_curr_task_fkt")) {
			...
			// Only on PROOSEK
			global_add("#if defined(__PROOSEK__) && !defined(OSEKOS_USE_COV)\n");
			global_add("\treturn keso_task_index[OSEKOStidact];\n");
			global_add("#else\n");
	...
	private void addTaskCode(PrintStream out, IMMethod mainMethod, TaskDefinition task, IMClass mainClass) throws CompileException {
		...
		if (opts.getGlobalHeap().deferTasksOnDemand()) {
			out.print("\tKESO_GC_DEFER_TASK();\n");
		}
	...
	public void writeMainFile() throws CompileException {
		...
		if (os!=null) {
			...
			if (tricore_PMI_workaround) {
				out.print("\n\n/*\n * Workaround for TC1796 and Trace 32:");
				out.print("\n * Bug: Program Memory Interface (PMI) line buffer not invalidated during CPU halt.\n */\n");
				out.print("static short flush_data[9] __attribute__ ((unused));\n");
				out.print("void flush_PMI_buffer() {\n");
				out.print("\t__asm__ __volatile__ (\".align 4; debug; nop; nop; nop; nop; nop; nop; nop; nop; nop; nop;\");\n");
				out.print("}\n");
			}
